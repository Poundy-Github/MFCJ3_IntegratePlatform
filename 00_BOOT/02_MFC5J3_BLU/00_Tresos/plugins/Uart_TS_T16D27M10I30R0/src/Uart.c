...
#if (UART_SW_MAJOR_VERSION != 10U)
#error "UART_SW_MAJOR_VERSION does not match. "
#endif

#if (UART_SW_MINOR_VERSION != 30U)
...
LOCAL_INLINE void Uart_lReceiveError(\
...
LOCAL_INLINE void Uart_lRead(Ifx_ASCLIN *const HwModulePtr,\
                          Uart_ChannelInfoType *const ChannelInfoPtr,\
                          const Uart_ChannelConfigType *const ChannelConfigPtr,\
                          const uint8 ReceiveFillLevel);
LOCAL_INLINE void Uart_lEnableWriteInterrupts(Ifx_ASCLIN *const HwModulePtr);
LOCAL_INLINE void Uart_lHwInit(const uint8 HwUnit,\
                          const Uart_ChannelConfigType *const ChannelConfigPtr);
LOCAL_INLINE void Uart_lWrite(Ifx_ASCLIN *const HwModulePtr,\
                          Uart_ChannelInfoType *const ChannelInfoPtr,\
                          const Uart_ChannelConfigType *const ChannelConfigPtr);
...
#if(UART_RX_POLLING_ENABLE == STD_ON)
...
void Uart_MainFunction_Read(void)
{
  ...
  #endif
  {
    ...
    for(ChannelNo = 0U; ChannelNo < MaxChannel; ChannelNo++)
    {
      ...
      if((ChannelConfigPtr->RxMode == UART_RX_POLLING) && \
...
#if(UART_TX_POLLING_ENABLE == STD_ON)
/*******************************************************************************
...
void Uart_MainFunction_Write(void)
{
  ...
  #endif
  {
    ...
    for(CfgCount = 0U; CfgCount < MaxChannel; CfgCount++)
    {
      ...
      if((ChannelConfigPtr->TxMode == UART_TX_POLLING) && \
                          ...
                          const Uart_SizeType Size)
{
  ...
  #endif
  {
    ...
    if(Uart_ConfigPtr->ChannelConfigPtr[Channel].RxMode != UART_RX_POLLING)
    #endif
    {
...
** Syntax : LOCAL_INLINE void Uart_lRead                                      **
**  (                                                                         **
**    Ifx_ASCLIN * const HwModulePtr,const Uart_ChannelIdType Channel         **
**  )                                                                         **
**                                                                            **
** Service ID       : NA                                                      **
**                                                                            **
...
** Syntax : LOCAL_INLINE static void Uart_lEnableReadInterrupts (Ifx_ASCLIN*  **
**                                                        const HwModulePtr)  **
**                                                                            **
** Service ID      : NA                                                       **
**                                                                            **
...
LOCAL_INLINE void Uart_lEnableReadInterrupts(Ifx_ASCLIN *const HwModulePtr)
{
  ...
  [/cover] */
  /* Enable Parity error */
  HwModulePtr->FLAGSENABLE.B.PEE = UART_SET_BIT;
  /* [cover parentID={029D69E3-0D5B-4926-9BDB-EE3A47FBC03F}]
  [/cover] */
  /* Enable Frame error */
  HwModulePtr->FLAGSENABLE.B.FEE = UART_SET_BIT;
  /* [cover parentID={9641E3A4-2146-4642-91EF-D3C11E998CDD}]
  [/cover] */
  /* Enable  RxFIFO level interrupt */
  HwModulePtr->FLAGSENABLE.B.RFLE = UART_SET_BIT;
  /* [cover parentID={2693EEAE-1C1E-4d79-B507-CF885D041BC8}]
  [/cover] */
  /* Enable RXFIFO Overflow error */
  HwModulePtr->FLAGSENABLE.B.RFOE = UART_SET_BIT;
}
...
**                   ( Ifx_ASCLIN *const HwModulePtr,                         **
**                     Uart_ChannelInfoType *const ChannelInfo,               **
**                     const Uart_ChannelConfigType *const ChannelConfigPtr   **
**                   )                                                        **
**                                                                            **
** Service ID      : NA                                                       **
...
** Syntax          : LOCAL_INLINE void Uart_lReceiveError                     **
**                   (                                                        **
**                      const Uart_ChannelConfigType *const ChannelConfigPtr, **
**                      Ifx_ASCLIN *const HwModulePtr,                        **
**                      Uart_ChannelInfoType *const ChannelInfoPtr            **
...
** Syntax           : LOCAL_INLINE void Uart_lHwInitKernelReg                 **
**                      (Ifx_ASCLIN *const HwModulePtr)                       **
**                                                                            **
** Service ID       : NA                                                      **
**                                                                            **
...
** Syntax           : LOCAL_INLINE void Uart_lHwInit(const uint8 HwUnit,      **
...
** Syntax           : static uint8 Uart_lStatusTimeout                        **
...
**                    ExpStatValue - Expected ststus bit value.               **
** Parameters (out) : None                                                    **
**                                                                            **
** Return value     : None                                                    **
**                                                                            **
...
LOCAL_INLINE void Uart_lDisableReadInterrupts(Ifx_ASCLIN *const HwModulePtr)
{
  ...
  [/cover] */
  /* Disable Parity errror */
  HwModulePtr->FLAGSENABLE.B.PEE = UART_CLEAR_BIT;
  /* [cover parentID={60A6BAA2-448F-4e3e-B2C1-2019CB32FCF5}]
  [/cover] */
  /* Disable frame errror */
  HwModulePtr->FLAGSENABLE.B.FEE = UART_CLEAR_BIT;

  /* [cover parentID={84496265-7F0C-4f20-BFB4-A9205FBD7A7E}]
  [/cover] */
  /* Disable RXFIFO overflow errror */
  HwModulePtr->FLAGSENABLE.B.RFOE = UART_CLEAR_BIT;

  /* [cover parentID={D027E483-D16D-401f-A3E8-37625447DB03}]
  [/cover] */
  /* Disable RXFIFO level interrupts */
  HwModulePtr->FLAGSENABLE.B.RFLE = UART_CLEAR_BIT;
}
...
LOCAL_INLINE void Uart_lClearReadInterrupts(Ifx_ASCLIN *const HwModulePtr)
{
  ...
  [/cover] */
  /* Clear Parity errror */
  HwModulePtr->FLAGSCLEAR.B.PEC = UART_SET_BIT;
  /* [cover parentID={A8A2A551-2309-4d43-BB6A-CF09721D35FE}]
  [/cover] */
  /* Clear frame errror */
  HwModulePtr->FLAGSCLEAR.B.FEC = UART_SET_BIT;

  /* [cover parentID={5AF00364-C9B4-4755-A124-B424027B74D9}]
  [/cover] */
  /* Clear RXFIFO overflow */
  HwModulePtr->FLAGSCLEAR.B.RFOC = UART_SET_BIT;

  /* [cover parentID={3B0E1AF9-2334-472f-87BD-B32B1C38E7F9}]
  [/cover] */
  /* Clear RXFIFO level interrupts */
  HwModulePtr->FLAGSCLEAR.B.RFLC = UART_SET_BIT;
}

/*******************************************************************************
** Traceability     : [cover parentID={B89977F1-117E-4821-A4D0-6E8D2EC27FDC}] **
** Syntax : LOCAL_INLINE void Uart_lWrite                                     **
**   (                                                                        **
**      Ifx_ASCLIN* const HwModulePtr,\                                       **
**        Uart_ChannelInfoType *const ChannelInfoPtr,                         **
**        const Uart_ChannelConfigType *const ChannelConfigPtr)               **
...
** Syntax : LOCAL_INLINE Uart_ReturnType Uart_lCheckDetError                  **
**                       (                                                    **
**                         uint8 Channel,                                     **
**                         Uart_SizeType Size,                                **
**                         uint8 ApiId,                                       **
