...
#define CANSM_CALL_SM_MODEINDICATION(nw,mode) \
   CANSM_NW_CONFIG(nw).ModeIndication(CANSM_NW_HANDLE_FROM_IDX(nw),mode)
[!ELSE!]
...
typedef P2FUNC( void, CANSM_CODE, CanSM_Nw_CallsFctPtrType )
  (
...
[!ELSE!] /* "$numNetworks > 1" */
[!IF "$multiCoreSupport = 'true'"!]
/** \brief if there's only one network, define CanSM_NetworkConfigType with only the SchM function pointer */
typedef struct {
  CanSM_Nw_CallsFctPtrType  ModeIndication;
} CanSM_NetworkConfigType;
[!ENDIF!]
[!ENDIF!]
...
/*==================[external constants]====================================*/
[!IF "$numNetworks > 1 or $multiCoreSupport = 'true'"!]
