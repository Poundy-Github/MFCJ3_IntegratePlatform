...
define build-exe-file
	$(info Linking $(notdir $@))
	$(EB_VERBOSE)$(LINK) -Map=$(notdir $(MAP_FILE)) -L$(subst \,/,$(BOARD_PROJECT_PATH)) @$(subst \,/,$(LNK_INC)) -T $(GET_LOC_FILE) -o $(subst \,/,$@)
	$(MV) $(PROJECT).map $(MAP_FILE)    
	$(EB_VERBOSE)$(OBJCPY) -O binary $(subst \,/,$@) $(subst \,/,$(addsuffix .bin, $(basename $@)))
	$(EB_VERBOSE)$(OBJCPY) -O srec $(subst \,/,$@) $(subst \,/,$(addsuffix .srec, $(basename $@)))
	@echo *** BUILD DONE ***
endef
...
define copy-platform-files
	...
	$(EB_VERBOSE)$(COPY) $(subst /,\,$(BOARD_PROJECT_PATH)/*.ldscript $(FLAT_MAK_DIR)) >null
	$(EB_VERBOSE)$(COPY) $(subst /,\,$(GENERATED_DIR)/*.ldscript $(FLAT_MAK_DIR)) >null
endef
