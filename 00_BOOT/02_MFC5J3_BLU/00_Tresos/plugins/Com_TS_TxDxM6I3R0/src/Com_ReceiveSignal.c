 ...
 * \param[inout] LengthPtr - in - length of the provided signal data buffer
   ...
   TS_PARAM_UNUSED(IPduPtr);
   {
      ...
      if (takeInitVal == TRUE)
      {
         ...
         if( (*LengthPtr) >= SigLen )
         {
            ...
            CONSTP2CONST(uint8, AUTOMATIC, COM_VAR_NOINIT) initDataPduPtr =
                     COM_GET_CONFIG_ELEMENT(uint8, IPduPtr->ComInitValueRef, 0U);

            CONSTP2CONST(uint8, AUTOMATIC, COM_VAR_NOINIT) initDataDynSigPtr =
...
)
{
   ...
   if (takeInitVal == TRUE)
   {
      ...
      CONSTP2CONST(uint8, AUTOMATIC, COM_VAR_NOINIT) initDataPtr =
               COM_GET_CONFIG_ELEMENT(uint8, IPduPtr->ComInitValueRef, 0U);
...
(COM_MAP_RECEIVE_SIGNAL == COM_RECEIVE_SIGNAL_GENERATED)) */
{
   ...
   if (SignalPtr->ComSignalBase.ComBufferRef == COM_RAM_SIZE_MAX)
   {
