...
(
   P2VAR(PduR_GateTpSessionInfoType, AUTOMATIC, PDUR_VAR_NOINIT) pSessionInfo
);
...
(
   PduR_PduIdType GMPduId  /* TODO: check, if there is a need to be the first PDU-ID of the gateway routing paths with the same source (for multicore)!! */
);
#endif /* (PDUR_TPGATEWAY_SUPPORT == STD_ON) */
#endif /* (PDUR_ZERO_COST_OPERATION_TP == STD_OFF) */
