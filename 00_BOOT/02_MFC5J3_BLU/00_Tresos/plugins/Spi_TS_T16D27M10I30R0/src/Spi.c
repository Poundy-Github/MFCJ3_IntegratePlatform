...
#if(SPI_SW_MAJOR_VERSION != 10U)
#error "SPI_SW_MAJOR_VERSION does not match."
#endif

#if(SPI_SW_MINOR_VERSION != 30U)
...
#define SPI_LOOPBACK_SHIFT         (14U)
#define SPI_INVALID_LOOPBACK (0xFFU)
/*******************************************************************************
**                         User Mode Macros                                   **
*******************************************************************************/
...
/* MISRA2012_RULE_4_9_JUSTIFICATION: Function like macro is defined for
 User mode support in code. No side effects foreseen by violating this MISRA
 rule. */
#define SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG(RegAdd,Data)    \
    Mcal_WritePeripEndInitProtReg(RegAdd,Data)
#else
/* MISRA2012_RULE_4_9_JUSTIFICATION: Function like macro is defined for
   User mode support in code. No side effects foreseen by violating this MISRA
...
/* This holds the loopback status for each Kernel */
/* MISRA2012_RULE_9_3_JUSTIFICATION: explicit initilisation of array */
static uint32 Spi_KernelLoopBackState[SPI_MAX_HW_UNIT];
...
*******************************************************************************/
#if (SPI_MULTICORE_ERROR_DETECT == STD_ON)|| (SPI_SAFETY_ENABLE == STD_ON)
/*[cover parentID={775C916C-9F17-44e7-B213-7D8F8472F530}]*/
static void Spi_lReportMulticoreError(const uint8 ApiId,
                                      const uint8 ErrorId)
{
...
static Std_ReturnType Spi_lCheckCoreVar(const uint32 CoreId)
{
  ...
  else
  {
    ...
    {
        /*[cover parentID={8B1F0358-CEA9-4dc5-B604-28E9B2ECD885}][/cover]*/
...
void Spi_Init(const Spi_ConfigType* const  ConfigPtr)
{
  ...
  #endif /* SPI_DEV_ERROR_DETECT == STD_ON */
  {
    ...
    for(HwMap = 0; HwMap < SPI_MAX_HW_UNIT; HwMap++)
    {
        /* Initialize the loopback state of kernel */
        Spi_KernelLoopBackState[HwMap] = SPI_INVALID_LOOPBACK;
...
** Sync/Async      : Synchronous                                              **
**                                                                            **
** Reentrancy      : Reentrant  per QSPI HW                                   **
**                                                                            **
** Parameters (in) :   HWUnit - QSPI Hw Unit                                  **
...
#endif
{
    ...
    is parameter EnableOrDisable is valid
     [/cover] */
    if ((EnableOrDisable == SPI_LOOPBACK_ENABLE) || (EnableOrDisable == SPI_LOOPBACK_DISABLE))
    {
        /* [cover parentID={913199B3-F1F7-490a-A9CE-7330E8EF758D}]
        Is QSPI Hw number is valid
        [/cover] */

        if(HWUnit < (uint8)SPI_MAX_HW_UNIT)
        {
            /* [cover parentID={19DADE27-7F57-41f3-B77B-DCDC33213409}]
            ...
            if(Spi_lIsQSPIHwConfigured(HWUnit) == (uint8)E_OK)
            {
                ...
                if(SPI_HW_MODULE[HWUnit].STATUS.B.PHASE == 0U)
                {
                    ...
                    {
                        GlobalConReg.U = (GlobalConReg.U | (1UL << (uint32)SPI_LOOPBACK_SHIFT));
                    }
                    else
                    {
                        GlobalConReg.U = (GlobalConReg.U & ~(1UL << (uint32)SPI_LOOPBACK_SHIFT));
                    }
                    Spi_KernelLoopBackState[HWUnit] = (uint32)EnableOrDisable;
                    /* MISRA2012_RULE_11_5_JUSTIFICATION: Agreed violation - QSPI
                     register access */
                    /* MISRA2012_RULE_11_3_JUSTIFICATION: QSPI base address access */
                    /* MISRA2012_RULE_11_8_JUSTIFICATION: SFR access. Since the input
                    ...
                    * by violating this MISRA rule */
                    SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[HWUnit]. \
...
void Spi_QspiDmaCallout(const uint32 Channel, const Dma_EventsType Event)
{
  uint8 ModIndex;
...
static void Spi_lIsrDmaQspiRx(const uint8 Module)
{
  ...
  /* [cover parentID={C12EB726-24F2-48b9-B30D-9A220C185FAC}] Check if
   parameter is valid
  [/cover] */
...
** Reentrancy      : Non Reentrant                                            **
**                                                                            **
** Parameters (in) : Module - QSPI module index [0 – SPI_MAX_HW_UNIT]         **
**                   ApiId - Service ID of called function                    **
**                   SpiPt2Status - Represents PT2 enable status              **
**                                                                            **
** Parameters (out): None                                                     **
**                                                                            **
** Return value    : None                                                     **
                                           ...
                                           const uint8 SpiPt2Status)
{
  ...
  Spi_JobType JobId;
    ...
    {
      /* [cover parentID={DAB61C68-A7F5-4a89-8D7C-71DAF13BE214}]
      Spurious interrupt - DET
      [/cover] */
      /* ISR invalid ISR DET */
      Spi_lReportError(ApiId, \
...
void Spi_IsrQspiPT2(const uint8 Module)
{
  ...
  if(Module < SPI_MAX_HW_UNIT)
  {
    ...
    /* MISRA2012_RULE_11_5_JUSTIFICATION: Agreed violation - QSPI
       register access */
    SpiPt2ENStatus = (uint8) (SPI_HW_MODULE[Module].GLOBALCON1.B.PT2EN);
    SpiPt2Status  = (SpiPt2Status & SpiPt2ENStatus);
    Spi_lQspiTriggerNextJob(Module, SPI_SID_PT2_INTERRUPT, SpiPt2Status);
  }
  #if(SPI_SAFETY_ENABLE == STD_ON)
  else
  {
...
static void Spi_lErrorHandler(const uint8 Module)
{
   ...
   * argument for the API discards volatile keyword. No side effects foreseen
   * by violating this MISRA rule */
  SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[Module]. \
                                        GLOBALCON.U, GlobalConReg.U);
  ...
  else
  {
    ...
    if((Spi_RuntimeCoreVar[CoreId] != NULL_PTR))
    {
      ...
      for(LoopIndex = 0U; LoopIndex < SPI_MAX_HW_UNIT; LoopIndex++)
      {
        ...
        {
          /* [cover parentID={9CC9C8F7-DFE1-4631-BFEE-3B86146BB36D}]
          Asynchronous status is un-initialized
          [/cover] */
...
static void Spi_lCoreInit(const uint32 CoreId)
{
  ...
  switch(CoreId)
  {
    case CORE0:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore0, sizeof(Spi_SequenceStatusCore0));
    ...
    case CORE1:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore1, sizeof(Spi_SequenceStatusCore1));
    ...
    case CORE2:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore2, sizeof(Spi_SequenceStatusCore2));
    ...
    case CORE3:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore3, sizeof(Spi_SequenceStatusCore3));
    ...
    case CORE4:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore4, sizeof(Spi_SequenceStatusCore4));
    ...
    case CORE5:
      ...
      Spi_lClearMem((uint8*)Spi_SequenceStatusCore5, sizeof(Spi_SequenceStatusCore5));
...
static void Spi_lQSPIHwResetInit(const uint8 ModIndex)
{
   ...
   * argument for the API discards volatile keyword. No side effects foreseen
   * by violating this MISRA rule */
  SPI_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[ModIndex].\
  ...
  for(LoopIndex = 0U; LoopIndex < SPI_MAX_ECON_CHANNEL; LoopIndex++)
  {
     ...
     * argument for the API discards volatile keyword. No side effects foreseen
     * by violating this MISRA rule */
    SPI_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[        \
...
static void Spi_lSyncStartJob(const Spi_JobType JobId)
{
  ...
  do
  {
    ...
    else
    {
       ...
       * argument for the API discards volatile keyword. No side effects
       foreseen by violating this MISRA rule */
      SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[HwId]. \
                                            GLOBALCON.U, GlobalConReg.U);
      /* Exit tx loop */
...
)
{
  ...
  /* [cover parentID={8E837A82-BC83-4697-91E6-D5EED23D50B1}]
     Check for Loopback[/cover] */
  if (Spi_KernelLoopBackState[HwUnitNum] != SPI_INVALID_LOOPBACK)
  {
      if (Spi_KernelLoopBackState[HwUnitNum] == (uint32)SPI_LOOPBACK_DISABLE)
      {
         GlobalConReg.U = (GlobalConReg.U & ~(1UL << (uint32)SPI_LOOPBACK_SHIFT));
      }
      else
  ...
  if ((Spi_CoreConfigPtr->QSPIHwConfigPtr[HwUnitNum]->ExternalDemuxEnabled) == 1U)
  {
      Strobe = Spi_CoreConfigPtr->QSPIHwConfigPtr[HwUnitNum]->StrobeDelay;
      /* Set DEL0 bit */
  ...
  #if defined (SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG)
  ...
  * argument for the API discards volatile keyword. No side effects foreseen
  * by violating this MISRA rule */
  SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG \
   ...
   * argument for the API discards volatile keyword. No side effects foreseen
   * by violating this MISRA rule */
  SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&ModulePtr->GLOBALCON1.U,    \
                                        GlobalCon1Reg.U);
  /* MISRA2012_RULE_11_5_JUSTIFICATION: Agreed violation - QSPI
      register access */
  /* MISRA2012_RULE_11_3_JUSTIFICATION: QSPI base address access */
   ...
   * argument for the API discards volatile keyword. No side effects foreseen
   * by violating this MISRA rule */
  SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&ModulePtr->ECON[HwChannelno \
...
/*******************************************************************************
**                                                                            **
** Traceability     :[cover parentID={CD7B9959-72C7-4c09-A1DA-8CCACFED8669}] **
...
static void Spi_lUpdateQspiIntr(const uint8 Enable)
{
  ...
  for(Modid = 0U; Modid < SPI_MAX_HW_UNIT; Modid++)
  {
    ...
    if(Spi_lIsQSPIHwConfiguredAsync(Modid) == (uint8)E_OK)
    {
      ...
      * argument for the API discards volatile keyword. No side effects foreseen
      * by violating this MISRA rule */
      SPI_RUNTIME_WRITE_PERIP_ENDINIT_PROTREG((uint32*)&SPI_HW_MODULE[Modid]. \
                                            GLOBALCON1.U, GlobalCon1Reg.U);
    }
  }
