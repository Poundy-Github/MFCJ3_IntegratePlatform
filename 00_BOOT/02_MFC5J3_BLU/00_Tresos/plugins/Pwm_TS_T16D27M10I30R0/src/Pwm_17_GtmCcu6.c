...
#if ( PWM_17_GTMCCU6_SW_MAJOR_VERSION != 10U )
#error "PWM_17_GTMCCU6_SW_MAJOR_VERSION does not match. "
#endif

#if ( PWM_17_GTMCCU6_SW_MINOR_VERSION != 30U )
#error "PWM_17_GTMCCU6_SW_MINOR_VERSION does not match. "
#endif

#if ( PWM_17_GTMCCU6_SW_PATCH_VERSION != 1U )
...
#if((PWM_17_GTMCCU6_NOTIFICATION_SUPPORTED == STD_ON)&& \
    (PWM_17_GTMCCU6_SAFETY_ENABLE == STD_ON))
/*The position of notif flag to enable GTM interrupts to trigger DSADC */
...
#if (PWM_17_GTMCCU6_DE_INIT_API == STD_ON)
#if (PWM_17_GTMCCU6_GTM_TIMER_USED == STD_ON)
  #define PWM_TOM_CTRL_RST_MSK     ((uint32)0xFEEF8FFFU)
  #define PWM_ATOM_CTRL_RST_MSK     ((uint32)0xFEEF8FFCU)
#endif
#endif
...
#if (PWM_17_GTMCCU6_NOTIFICATION_SUPPORTED == STD_ON)
#if (PWM_17_GTMCCU6_CCU6_TIMER_USED == STD_ON)
LOCAL_INLINE uint8 Pwm_lHandlePeriodMatch(uint8 Ccu6Kernel, uint8 \
    Ccu6Comparator, uint8 CoreId);
#endif
#if (PWM_17_GTMCCU6_SAFETY_ENABLE == STD_ON)
LOCAL_INLINE uint32 Pwm_lUnInitChIdCheck(Pwm_17_GtmCcu6_ChannelType LogicalChId,\
                                        uint8 CoreId);
#endif
...
#endif
{
  ...
  #if (PWM_17_GTMCCU6_SAFETY_ENABLE == STD_ON)
        /* [cover parentID={38AFF32A-ED3F-43b6-BEF6-65DA954A0878}]
  [/cover] */
...
/*CYCLOMATIC_Pwm_17_GtmCcu6_EnableNotification_JUSTIFICATION: This Function has 
  cyclomatic complexity >15. Multiple conditions needed to verify DSADC triggered
  PWM channel notification .Further breakdown cannot be done in order to maintain 
  readability.*/
...
)
{
  ...
  Is error reported by lVerUnInitAndChannel
  [/cover] */
    ...
    #endif
    {
      ...
      if((Dsadc_NotifVal == PWM_CONST_1) && (Notification != PWM_CONST_3))
      {
        ...
        Mcal_ReportSafetyError(
          ...
          PWM_17_GTMCCU6_INSTANCE_ID,
          PWM_17_GTMCCU6_SID_ENABLENOTIFI,
          PWM_17_GTMCCU6_E_INVALID_EDGE_NOTIF);/* End of report to Safety */
        ErrStatus = (uint8)E_NOT_OK;
      }
      #endif
    }
...
(const Pwm_17_GtmCcu6_ChannelType ChannelNumber)
{
  ...
  [/cover] */
   /* [cover parentID={37FC1A06-BD0B-4ea4-BA66-D0EB8579E811}]
  [/cover] */
  if ( PWM_ERROR_FOUND ==
  ...
  #endif
  {
    ...
    If Idle state and polarity mismatches
    [/cover] */
    if (IdleState != Polarity)
    {
      /* Idle state and Polarity mismatches then 0 % duty desired */
...
)
{
  ...
  Is error status is OK
  [/cover] */
  /* [cover parentID={8A407950-106E-4ebc-BDFF-C0819F830741}]
  [/cover] */
  if(ErrorStatus == E_OK)
  {
    /* Call GTM Library to Initialize TOM channel */
...
**                from Unintended shadow transfers which may occur            **
...
)
{
  ...
  /* [cover parentID={4BC6D644-0271-4c7d-8DCA-5C3560734828}]
  [/cover] */
  ...
  /* [cover parentID={7E753AD3-DC86-4b9c-8CD1-EEA22ED798C2}]
  [/cover] */
 ...
 const uint8 TimerChannelNumber, const Pwm_17_GtmCcu6_OutputStateType IdleState)
{
  ...
  /* Reset GTM TOM CTRL TRIGOUT, CLK_SRC_SR and RST_CCU0 register bits*/
  ...
  Clear all pending interrupts. Disable Interrupts
  [/cover] */
  TomChannelRegPtr->IRQ.NOTIFY.U = PWM_CLR_PENDING_INTERRUPTS;
 ...
 const uint8 TimerChannelNumber, const Pwm_17_GtmCcu6_OutputStateType IdleState)
{
  ...
  [/cover] */
 /* Reset GTM ATOM CTRL TRIGOUT, CLK_SRC_SR, MODE and RST_CCU0 register bits*/
...
)
{
  ...
  /* [cover parentID={709FEA81-DCAD-43cf-AD5D-5F852CB065CA}]
  [/cover] */
...
)
{
  ...
  [/cover] */
  ErrorFlag = Pwm_lUnInitChIdCheck(LogicalChId,(uint8)CoreId);
  ...
  #endif
  {
        ...
        PWM_NOTIFI_STATUS_MASK) == PWM_NOTIFI_STATUS_MASK)
    {
      ...
      if(PwmTimerType == PWM_17_GTMCCU6_GTM_TIMER)
      {
       /* [cover parentID={502CF639-6051-43ff-8D0B-5470E6B101B1}]
       [/cover] */
...
#if (PWM_17_GTMCCU6_CCU6_TIMER_USED == STD_ON)
/******************************************************************************
** Syntax :          uint8 Pwm_lHandlePeriodMatch(uint8 Ccu6Kernel,uint8\    **
**                                     Ccu6Comparator,uint8 CoreId)          **
**                                                                           **
** Service ID:       NA                                                      **
**                                                                           **
...
#if (PWM_17_GTMCCU6_SAFETY_ENABLE == STD_ON)
/******************************************************************************
** Syntax :         uint32 Pwm_lUnInitChIdCheck(Pwm_17_GtmCcu6_ChannelType   **
