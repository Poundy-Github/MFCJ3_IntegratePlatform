...
typedef struct
{
  ...
  /* flag for rounding up with +1 the STmin */
  VAR(boolean, TYPEDEF) STminRoundUpFlag;
  /* channel state */
  ...
  /* flag indicating reception ongoing for a N_AI on current channel */
  VAR(boolean, TYPEDEF) RxChannelLockFlag;
  VAR(boolean, TYPEDEF) TxLastFrameFlag;
#if(CANTP_N_BR_WAIT_SUPPORT == STD_ON)
  /* flag indicating the previous sent FC was with WAIT status */
  VAR(boolean, TYPEDEF) FcWaitRepeatedFlag;
#endif
} CanTp_ChannelType;
