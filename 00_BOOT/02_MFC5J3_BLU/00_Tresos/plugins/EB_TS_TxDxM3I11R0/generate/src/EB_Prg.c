...
NCS_PRI_VAR const u8 aubCOMRxFrame[COM_RX_FRAME_MAX] =
    {
 ...
 **/
NCS_PRI_FCT u8 EB_GetMessageIdBySourceAddress(u8 ubSourceAddress, tTpMsgIdx * uMsgIdx);
...
[!ENDIF!]
        ...
        if (eStatusMsgData == TP_ERR_OK)
        {
            ...
            eStatusRxReq = UDS_RxRequestWithAddrMode (&uwLen, paubUdsData, ubAdressingMode, ubAddressingType);
...
[!IF "as:modconf('BM')/General/BM_SOURCE_ADDRESS_CHECK = 'true'"!]
/*----------------------------{EB_SetTesterAddress}------------------------------*/
void EB_SetTesterAddress(u8 ubTesterAddress)
{
...
void EB_SimulateRxRequest(u16 uwLen, u8 * paubUdsData, u8 ubWithResp)
{
...
[!ENDIF!]
    DBG_EB_SIMULATERXREQUEST_ENTRY(uwLen, paubUdsData, ubWithResp);
...
[!IF "as:modconf('BM')/General/BM_SOURCE_ADDRESS_CHECK = 'true' and as:modconf('EB')/General/Osek_Autosar_Stack = 'OSEK'"!]
    ...
    eUdsBoolean = UDS_RxRequest(&uwLen, m_paubTpBuffer);
