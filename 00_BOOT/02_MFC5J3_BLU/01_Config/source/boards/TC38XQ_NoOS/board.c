...
extern char __STARTFLASHDRIVERROM, __STARTFLASHDRIVERRAM, __ENDFLASHDRIVERRAM;
...
static void BoardCopyRomToRam(void)
{
    ...
    for ( p = &__STARTFLASHDRIVERRAM, q = &__STARTFLASHDRIVERROM; p < &__ENDFLASHDRIVERRAM; p++, q++ )
    {
        *p = *q;
    }
 ...
 */
void BoardInvalidException(uint32 id, uint32 sp)
{

  LOG(LOG_LEVEL_ERROR, "BoardInvalidException id[%X] sp[%X]",id, sp);
