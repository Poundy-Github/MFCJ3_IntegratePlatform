...
NCS_PRI_VAR tProgDecompressStateType m_uProgDecompStatus;
...
/*---------------------------{PROG_DecompressData}-------------------------------*/
tProgStatus PROG_DecompressData(u8* pubData, u16 uwDataLength)
{
    Cal_ReturnType eCplStatus;
    tProgStatus eProgStatus;
    u32 ulOutputLength;
    ...
    DBG_PROG_DECOMPRESSDATA_ENTRY(pubData, uwDataLength);

    m_stInBuffContext.ulBufLength += uwDataLength;

    if (m_stInBuffContext.ulBufLength != 0)
    {
        if (NULL_PTR != pubData)
        {
...
tProgDecompressStateType PROG_GetDecompressedData(u8** pubDecompData,
    u16* puwDecompressLength)
{
    tProgDecompressStateType eProgDecompState;
    u16 uwIdx;

    DBG_PROG_GETDECOMPRESSEDDATA_ENTRY(pubDecompData, puwDecompressLength);
    ...
    if((m_uProgDecompStatus == PROG_DECOMP_IN_PROGRESS) || (m_uProgDecompStatus == PROG_DECOMP_FINISH))
    {
        if ((m_stOutBuffContext.ulBufPosition < m_stOutBuffContext.ulBufLength))
        {
            ...
            *puwDecompressLength = (u16)(m_stOutBuffContext.ulBufLength - m_stOutBuffContext.ulBufPosition);

            if((*puwDecompressLength != (u16)m_stOutBuffContext.ulBufLength) && (*puwDecompressLength < PROG_MIN_VAL_TO_WRITE))
            {
                ...
                for(uwIdx = 0U; uwIdx < *puwDecompressLength; uwIdx++)
                {
                ...
                {
                    m_stOutBuffContext.aubDecompPageBuffer[uwIdx] = m_stOutBuffContext.aubBuf[uwIdx-*puwDecompressLength];
                }

                /* return pointer to data to write */
                *pubDecompData = m_stOutBuffContext.aubDecompPageBuffer;
                *puwDecompressLength = uwIdx;
            }
            else
            {
                /* return pointer to data to write */
            ...
            if(*puwDecompressLength != (u16)m_stOutBuffContext.ulBufLength)
            {
        ...
        {
            /* return the length of available data */
            *puwDecompressLength = (u16)m_stOutBuffContext.ulBufLength;
            /* return pointer to data to write */
