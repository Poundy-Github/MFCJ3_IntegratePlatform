  ...
  CANNM_PDL_SF(const uint8 instIdx))
{
  ...
  /* !LINKSTO CANNM129,1, CanNm.EB.Dsn.StateDynamics.NET_REQSTATUS_CHANGED_BUS_SLEEP_MODE,1 */
  ...
  CANNM_PDL_SF(const uint8 instIdx))
{
  ...
  {
    TS_AtomicSetBit_8(&CANNM_CHANNEL_STATUS(instIdx).ChanStatus, CANNM_IMMEDIATE_TRANS_BIT);
  }
  ...
  if ((CANNM_CHANNEL_STATUS(instIdx).ChanStatus & CANNM_COM_DISABLED) == 0U)
  {
...
#endif
    {
      if(((CANNM_CHANNEL_STATUS(instIdx).ChanStatus & CANNM_NETWORK_REQUESTED) != 0) ||
          (CANNM_CHANNEL_CONFIG(instIdx).RMSTime != 0))
      {
...
FUNC(void, CANNM_CODE) CanNm_HsmCanNmSfNormalOperationRemoteSleepEntry(
  CANNM_PDL_SF(const uint8 instIdx))
{
  /* entry action '#ifdef(RSI) RsiStatus = CANNM_RSI_TRUE;' */
#if (CANNM_REMOTE_SLEEP_IND_ENABLED == STD_ON)
...
FUNC(void, CANNM_CODE) CanNm_HsmCanNmSfRepeatMessageStateEntry(
  CANNM_PDL_SF(const uint8 instIdx))
{
  /* entry action '#ifdev (SCI) Nm_StateChangeNotification(LastState, ActState);CurState = RepeatMessageState; RMSTimerStart() -->CANNM102;' */
 ...
 * 2) PrepareBusSleepMode -> NetworkMode: NET_REQ_STATUS_CHANGED[NetworkRequested==TRUE]/#if (CANNM_IMM._RESTART == true) <Send NM message> #endif
  ...
  CANNM_PDL_SF(const uint8 instIdx))
{
  ...
  if(CanIf_Transmit(CANNM_CHANNEL_TXPDUID(instIdx), &pduInfo) == E_OK)
  {
 ...
 CANNM_PDL_SF(const uint8 instIdx))
{
  ...
  else
  {
     ...
     */
    TS_AtomicClearBit_8(&CANNM_CHANNEL_STATUS(instIdx).ChanStatus, CANNM_IMMEDIATE_TRANS_BIT);
