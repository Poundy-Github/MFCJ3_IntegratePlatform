...
/* patch version check */
#if (CANNM_SW_PATCH_VERSION < 2U)
#error CANNM_SW_PATCH_VERSION wrong (< 2U)
#endif
...
STATIC FUNC(void, CANNM_CODE) CanNm_HandleCarWakeup(
  CANNM_PDL_SF(const PduIdType instIdx));
#endif
...
(
  CONSTP2VAR(CanNm_PnStatusType, AUTOMATIC, CANNM_APPL_CONST) PnStatePtr,
  CONST(PduIdType, CANNM_CONST) PduId
);
...
(
    CONSTP2VAR(CanNm_PnStatusType, AUTOMATIC, CANNM_VAR) pPnStatus,
    CONST(PduIdType, CANNM_CONST) PduId
);
...
VAR(CanNm_ChanStatusType, CANNM_VAR)
  CanNm_ChanStatus[CANNM_NUMBER_OF_CHANNELS];

P2CONST(CanNm_ConfigType, CANNM_VAR, CANNM_APPL_CONST) CanNm_RootPtr;
...
VAR(CanNm_PnStatusType, CANNM_VAR) CanNm_EiraStatus;
#endif
...
)
{
...
#endif
  {
    ...
    TS_AtomicSetBit_8(&CANNM_CHANNEL_STATUS(index).ChanStatus, CANNM_NET_REQUESTED_BIT);
...
)
{
...
#endif
  {
    ...
    TS_AtomicClearBit_8(&CANNM_CHANNEL_STATUS(index).ChanStatus, CANNM_NET_REQUESTED_BIT);
...
#if ((CANNM_COM_USER_DATA_SUPPORT == STD_ON) || (CANNM_PN_EIRA_CALC_ENABLED == STD_ON))
FUNC(void, CANNM_CODE) CanNm_GetPduUserData(const uint8 instIdx,
