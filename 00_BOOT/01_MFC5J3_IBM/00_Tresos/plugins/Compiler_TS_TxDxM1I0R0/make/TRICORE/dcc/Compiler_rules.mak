...
define build-c-file
	...
	$(EB_VERBOSE)$(CC) @$(MAKE_INC) -o $(OBJ_DIR)/$(notdir $(basename $@)).o $<
	$(EB_VERBOSE)$(MOVE) $(OBJ_DIR)/$(notdir $(basename $@)).d $(DEPEND_DIR)/$(notdir $(basename $@)).d
	@if exist $(notdir $(basename $@)).s $(MOVE) $(notdir $(basename $@)).s $(OBJ_DIR)/$(notdir $(basename $@)).s
	@if exist $(notdir $(basename $@)).ir $(MOVE) $(notdir $(basename $@)).ir $(OBJ_DIR)/$(notdir $(basename $@)).ir
endef
