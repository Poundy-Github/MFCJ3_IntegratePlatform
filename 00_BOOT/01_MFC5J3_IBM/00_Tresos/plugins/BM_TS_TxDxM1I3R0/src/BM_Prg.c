...
NCS_PRI_VAR tBMFRStatus m_ubBMFRStatus;
NCS_PRI_VAR tBMNetworkStatus m_ubBMNetworkStatus;
NCS_PRI_VAR tBMCycleCount m_ubBMFRCycleOrigin;
#endif

#if (BM_TIMEOUT_CHECK == BM_TIMEOUT_CHECK_ON)
NCS_PRI_VAR tBMTimeout m_ubBMTimeoutCount;
#endif
...
NCS_PRI_VAR const uint8 m_aubExpectedBootloaderChecksum[BM_CHECKSUM_SIZE_IN_BYTES] = {0U};
...
#endif
    {
        ...
        if (BM_FALSE == eBootFromAppl)
        {
            ...
            if (BM_TRUE == eApplValid)
            {
                 ...
                 ubSoftExecutionAllowed = BM_VerifyApplicationIntegrity();
