...
#define PROG_SOFTWARE_PART_FINISHED         0xFF
/*-------------------------------------------------------------------------------*/
/* - Private Types                                                               */
...
NCS_PRI_VAR u32 m_ulVBTWrittenData;
/*===============================================================================*/
...
void PROG_Impl10_Entry_SignatureCheck(void)
{
   ...
   else
   {
    ...
    /* Read VBT */
    (void)PROG_MemDataAccess( m_astCfgMemory[stConfigSegment[m_stCurrentSegment.ubSegmentId].ubMemoryIdx].eMemoryType,PROG_MEM_OPERATION_TYPE_READ, ulVBTAddress,ulVBTSize, &m_aubDataToBeSigned[8]);
    ...
    (
        ...
        &m_stDiagMessageInfo.pubData[PROG_RC_CHK_MEMORY_SIGNATURE_INDEX]
    );
}
...
tProgStatus PROG_ExtractEraseParameters_Impl11(tSegmentType * pstSegment)
{
    ...
    if (stConfigSegment[pstSegment->ubSegmentId].ePartitionType == PROG_ESS_PARTITION)
    {
...
#endif /*(PROG_ERASE_CHECK_TYPE == PROG_BLOCK_ERASE_CHECK)*/
        {
            ...
            pstSegment->ulSize = (stConfigSegment[0U].ulEraseEndAddress -
    ...
    else
    {
        ...
        if (PROG_E_OK == eESSStatus)
        {
            ...
            {
                ubBlockId = 0xFFU;
                for (ubLoopIndex = 0; ((ubLoopIndex < ubBlockNbr) && (0xFFU == ubBlockId)); ubLoopIndex++)
                {
                ...
                else
                {
                    ...
                    pstSegment->ubLogicalBlockId = ubBlockId + 1;
#if (PROG_ERASE_CHECK_TYPE == PROG_BLOCK_ERASE_CHECK)
                    /* Check if logical block is already erased, index 0 is reserved for ESS */
      ...
      if( ((ulStartAddress > m_ulLastWrittenAddress) && (TRUE ==  m_ubVerificationTablePresent)) || (FALSE ==  m_ubVerificationTablePresent) )
      {
        ...
        if (PROG_E_OK == eESSStatus)
        {
            ...
            if (PROG_E_OK == eESSStatus)
            {
                ...
                for (ubLoopIndex = 0; ((ubLoopIndex < ubBlockNbr) && (0xFFU == ubBlockId)); ubLoopIndex++)
                {
                    ...
                    if ((PROG_E_OK == eESSStatus) && (ulStartAddress >= ulBlockAddr))
                    {
                        ...
                        if ((PROG_E_OK == eESSStatus) && ((ulStartAddress + ulMemorySize) <= (ulBlockAddr + ulBlockLength)))
                        {
                          ...
                          if( (m_ubSoftwarePartID !=PROG_SOFTWARE_PART_FINISHED) && (m_ubSoftwarePartID != (ubLoopIndex + 1)) )
                          {
                            eProgStatus = PROG_E_NOT_OK;
                          }
                          else
                ...
                {
                    /* Save Block index, add 1 as index 0 is reserved for ESS */
                    *pubBlockIndex = ubBlockId + 1;
                    m_ubSoftwarePartID = ubBlockId + 1;
                    /* update the address of the last written data */
                    m_ulLastWrittenAddress = ulStartAddress + ulMemorySize - 1;
                }
            }
            else
            {
...
tUdsStatus PROG_WriteProdKey(u16 *puwLen, u8 *aubUdsData)
{
    tUdsStatus eStatus;
...
tUdsStatus PROG_ReadProdKey(u16 *puwLen, u8 *aubUdsData)
{
    tUdsStatus eStatus;
...
void PROG_Impl10_Entry_WriteKeyFinished(void)
{
  ...
  for (ubLoopIndex = 0; ((ubLoopIndex < PROG_DIGEST_LENGTH)&&(PROG_E_OK == ubHashStatus)); ubLoopIndex++)
  {
       /* compare hash */
  ...
  if(PROG_E_OK == ubHashStatus)
  {
    ...
    ulExponent =  (u32)((u32)(m_stDiagMessageInfo.pubData[PROG_PROD_KEY_INDEX]) << 24);
    ulExponent |=  (u32)((u32)(m_stDiagMessageInfo.pubData[PROG_PROD_KEY_INDEX + 1]) << 16);
    ulExponent |=  (u32)((u32)(m_stDiagMessageInfo.pubData[PROG_PROD_KEY_INDEX + 2]) << 8);
    ulExponent |=  (u32)(m_stDiagMessageInfo.pubData[PROG_PROD_KEY_INDEX + 3]);
    Prog_CustomWriteKey(ulExponent, &m_stDiagMessageInfo.pubData[PROG_PROD_KEY_INDEX + PROG_MOD_LENGTH], &m_stDiagMessageInfo.pubData[PROG_PROD_KEY_HASH_INDEX]);
  ...
  *pulVBTAddress = PROG_SBL_VBT_START_ADDRESS;
   ...
   /* SBL variant */
    ...
    else
    {
       ...
       (void)Prog_GetEssLogicalBlockVerifTable(m_stCurrentSegment.ubLogicalBlockId - 1, pulVBTAddress);
    }
#endif
}
...
NCS_PRI_VAR tProgStatus PROG_CheckNonProgrammedAreas(void)
{
    ...
    /* Plus VBT*/
    for (uwIdx = 0U; uwIdx < uwDataBlockNbr + 1; uwIdx++)
#else
...
NCS_PRI_VAR void PROG_GetDataBlockNbrFromVBT(u16 *uwBlockNumber)
{
  ...
  u32 ulVBTAddress;
   ...
   /* SBL variant */
   /* Address of VBT is get from ESS, subtract one because the ess data is not included in the ess   */
   (void)Prog_GetEssLogicalBlockVerifTable(m_stCurrentSegment.ubLogicalBlockId - 1, &ulVBTAddress);
...
NCS_PRI_VAR void PROG_GetVBTFormat(u16 *uwFormatID)
{
  ...
  (*uwFormatID)  = (u16)((u16)(aubData[0]) << 8);
...
NCS_PRI_VAR tProgStatus PROG_VerifyVBTFormatAndLength(void)
{
  ...
  if(0 == m_stCurrentSegment.ubLogicalBlockId)
  {
    ...
    if(PROG_E_OK == ubProgStatus)
    {
      uwVBTFormat = (u16)((u16)aubData[0] << 8);
