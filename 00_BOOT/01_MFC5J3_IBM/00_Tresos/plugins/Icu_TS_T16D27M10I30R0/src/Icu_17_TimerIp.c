...
#if ( ICU_17_TIMERIP_AR_RELEASE_MAJOR_VERSION != 4U )
...
#if ( ICU_17_TIMERIP_SW_MAJOR_VERSION != 10U )
#error "ICU_17_TIMERIP_SW_MAJOR_VERSION does not match. "
#endif

#if ( ICU_17_TIMERIP_SW_MINOR_VERSION != 30U )
#error "ICU_17_TIMERIP_SW_MINOR_VERSION does not match. "
#endif

#if ( ICU_17_TIMERIP_SW_PATCH_VERSION != 1U )
...
#include "Icu_17_TimerIp_MemMap.h"
  ...
  * by violating this MISRA rule, as the variable is accessed via a
  * root pointer in different api's. */
/* MISRA2012_RULE_5_1_JUSTIFICATION: External identifiers going beyond 32 chars
    due to Autosar Naming constraints.*/
/* MISRA2012_RULE_5_2_JUSTIFICATION: External identifiers going beyond 32 chars
    due to Autosar Naming constraints.*/
  ...
  const uint16 NotifyInterval)
{
  ...
  /* Notify interval should be greater than zero to enable notification
    ...
    #endif
    {
      ...
      #endif
      {
        ...
        /* As per bugzilla 11480 the notification flag
          shall be enabled in Icu_17_TimerIp_EnableNotification API */
  ...
  const Icu_17_TimerIp_ChannelType Channel)
{
  ...
  If edge detection or time stamping api are available.
  [/cover] */
  #if (ICU_17_TIMERIP_EDGE_DETECT_API == STD_ON) || \
  (ICU_17_TIMERIP_TIMESTAMP_API == STD_ON)
  const Icu_17_TimerIp_ChannelConfigType *LocalIcuChanPtr; /* ROM pointer */
  ...
  const Icu_17_TimerIp_ChannelType Channel)
{
  ...
  uint8 Measure = 0U;
  #endif
  volatile Icu_17_TimerIp_ValueType Time;
  uint32* GlbIndexStatusPtr;
  uint16 CellUsed;
  uint8 CoreChIndex;
  uint8 Hwused;
      ...
      if (PrevMode != Mode)
      {
        ...
        if (ErrorFlag != ICU_ERROR_FOUND)
        {
               ...
               Channel = Channel + 1U)
          {
            ...
            if ((uint8) ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT
                == (uint8) (LocalIcuChanPtr[Channel].IcuProperties.MeasurementMode))
            {
              /* Check if the mode is sleep */
           ...
           Channel = Channel + 1U)
      {
        ...
        switch (IcuOption)
        {
          ...
          default:
            ...
            Mcu_17_Gpt12_TimerDeInit( \
                (uint8)(LocalIcuChanPtr[Channel].IcuProperties.AssignedHwUnitNumber) \
                    & ICU_LOWER_NIBBLE);
            #endif
            break;
        }/* End of switch */
    ...
    if(Icu_17_TimerIp_ConfigPtr->CoreConfig[CoreId] != NULL_PTR )
    {
      ...
      if(E_OK == ErrorFlag)
      {
             ...
             Channel = Channel + 1U)
        {
          ...
          [/cover] */
          if((((*GlbIndexStatusPtr & ICU_CHNL_EDGE_MASK) >> ICU_CHNL_EDGE_POS) != \
  ...
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  TempRegVal = 0U;
  /* Configure the TIM mode, enable and GPRx selection depending on mode
   * selected for channel */
  switch((uint32)LocalIcuChanPtr->IcuProperties.MeasurementMode)
  {
    /* [cover parentID={0D2AC265-D285-45cf-A948-8493991F6938}]
    If channel mode is signal measurement
    [/cover] */
    case (uint32)ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT:
      #if(ICU_17_TIMERIP_GTM_TIM_USED == STD_ON)
      TempRegVal |= ICU_GTM_TIM_MODE_TPWM << ICU_GTM_TIM_MODE_POS;
      TempRegVal |= \
          ((uint32)ICU_TIM_CTRL_GPR0_CNTS_SEL << ICU_TIM_CTRL_GPR0SEL_POS) | \
          ((uint32)ICU_TIM_CTRL_GPR1_CNT_SEL << ICU_TIM_CTRL_GPR1SEL_POS);
      ...
      {
        TempRegVal |= ICU_TIM_CTRL_ECNT_RESET_SET;
      }
      #endif
      break;
    /* [cover parentID={B6EDD492-1B57-4a7d-BEF3-250070FA2BB0}]
    If channel mode is edge detect
    [/cover] */
    case (uint32)ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT:
      #if(ICU_17_TIMERIP_GTM_TIM_USED == STD_ON)
      TempRegVal |= ICU_GTM_TIM_MODE_TIEM << ICU_GTM_TIM_MODE_POS;
      TempRegVal |= ICU_GTM_TIM_ENABLE_CHANNEL;
      #endif
      break;
    /* [cover parentID={D76E5A70-8F06-41c0-8A3C-1123552934F4}]
    If channel mode is timestamp
    [/cover] */
    case (uint32)ICU_17_TIMERIP_MODE_TIMESTAMP:
      #if(ICU_17_TIMERIP_GTM_TIM_USED == STD_ON)
      TempRegVal |= ICU_GTM_TIM_MODE_TIEM << ICU_GTM_TIM_MODE_POS;
      TempRegVal |= ((uint32)(LocalIcuChanPtr->IcuProperties. \
                     TimChannelGpr0InputSelect) << ICU_TIM_CTRL_GPR0SEL_POS);
      #endif
      break;
    /* [cover parentID={C6C3D8CB-5338-45fc-8FF0-6B1088CAA81C}]
    If channel mode is edge count
    [/cover] */
    default: /* ICU_17_TIMERIP_MODE_EDGE_COUNTER */
      #if(ICU_17_TIMERIP_GTM_TIM_USED == STD_ON)
      TempRegVal |= ICU_GTM_TIM_MODE_TIEM << ICU_GTM_TIM_MODE_POS;
  ...
  /*Read the config data to update GTM TIM channel control register */
  TempRegVal |=
  ...
  /* [cover parentID={346537B3-976D-4fea-8683-56FF917847ED}]
  Identify the ISL and DSL need to be configured deneding on default edge.
  [/cover] */
  if((uint8)ICU_17_TIMERIP_FALLING_EDGE ==
      (uint8)LocalIcuChanPtr->IcuProperties.DefaultStartEdge)
  {
    TempRegVal |= ICU_GTM_TIM_ISL_DSL_FALLING;
  }
  else if((uint8)ICU_17_TIMERIP_RISING_EDGE ==
          (uint8)LocalIcuChanPtr->IcuProperties.DefaultStartEdge)
  {
    TempRegVal |= ICU_GTM_TIM_ISL_DSL_RISING;
  }
  else
  {
    TempRegVal |= ICU_GTM_TIM_ISL_DSL_BOTH;
  }
  /* [cover parentID={8947C253-70DD-4d52-9702-A11701526ED2}]
  Set the TIM channel clock selection, Filter parameters and CICTRL values
  from configuration.
  [/cover] */
  ...
  TimCfgPtr.TimChIntEnMode = (uint8) ((uint32) ICU_GTM_RESET_INTERRUPTS
  ...
  TimCfgPtr.TimChExtendedCtrlReg = ICU_GTM_LOOKUPTABLE_NOTUSED;
  TimCfgPtr.TimerId = (uint32)ChannelNumber;
  TimCfgPtr.TimerId |= ((uint32)ModuleNumber) << ICU_MCU_TIM_MODULE_POS;
  /* [cover parentID={52B49AB4-C8F6-4978-AC56-6B77B1A38A66}]
  select Port pin to GTM TIM INSEL register
  [/cover] */
  /*selection of Port pin to GTM TIM INSEL register */
  ...
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  TempRegVal = 0U;
  /* Calculate the TIM control register value depending on mode */
  switch((uint32)LocalIcuChanPtr->IcuProperties.MeasurementMode)
  {
    case (uint32)ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT:
      TempRegVal |= ICU_GTM_TIM_MODE_TPWM << ICU_GTM_TIM_MODE_POS;
  ...
  [/cover] */
  ErrorFlag = Mcu_17_Gtm_TimChInitCheck(&TimCfgPtr);

  /* Get the SFR value for TIMINSEL Register for TIM Channel */
  SfrVal = ((MODULE_GTM.TIMINSEL[ModuleNumber].U) >> \
         (ICU_GTM_TIMINSEL_CH1SEL_OFF * ChannelNumber)) & ICU_GTM_TIMINSEL_MASK;
  /* [cover parentID={3CF9ED6B-8168-4778-B940-556C0B7BE1B3}]
  Check if selection of Port pin to GTM TIM INSEL register is correct and
  No Error flag set.
  [/cover] */
  /*Check for selection of Port pin to GTM TIM INSEL register */
  if((ErrorFlag != E_NOT_OK) && \
      (SfrVal != (LocalIcuChanPtr->IcuProperties.PinSelection)))
  {
    ErrorFlag = E_NOT_OK;
  }
  return ErrorFlag;
...
LOCAL_INLINE Std_ReturnType Icu_lCcu6ChannelInitCheck(
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  Mcu_17_Ccu6_TimerConfigType Ccu6CfgPtr;
  ...
  switch((uint32)LocalIcuChanPtr->IcuProperties.MeasurementMode)
  {
    case (uint32)ICU_17_TIMERIP_MODE_SIGNAL_MEASUREMENT:
          ...
          (uint8)(LocalIcuChanPtr->IcuProperties.MeasurementProperty))
      {
        if((uint8)ICU_17_TIMERIP_RISING_EDGE == \
            (uint8)(LocalIcuChanPtr->IcuProperties.DefaultStartEdge))
        {
          Ccu6CfgPtr.TimerModeSelectReg = (uint8)ICU_CCU_CAPTURE_MODE2;
        }
        else
        {
          Ccu6CfgPtr.TimerModeSelectReg = (uint8)ICU_CCU_CAPTURE_MODE3;
        }
      }
      /* All other signal measure property requires MODE1*/
      else
      {
        Ccu6CfgPtr.TimerModeSelectReg = (uint8)0;
      }
      break;
    case (uint32) ICU_17_TIMERIP_MODE_TIMESTAMP:
    ...
    default: /* ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT */
      ...
      Ccu6CfgPtr.TimerModeSelectReg = (uint8)ICU_CCU_CAPTURE_MODE1;
      break;
  }
  ...
  /* [cover parentID={77ABFD47-7F7A-4f50-AAC4-E1B5DCAB75A0}]
   ...
   [/cover] */
  ErrorFlag = Mcu_17_Ccu6_TimerInitCheck(&Ccu6CfgPtr);
  ...
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  uint32 LActiveEdge;
  Mcu_17_Gpt12_TimerConfigType Gpt12Ptr;
  Std_ReturnType ErrorFlag = E_NOT_OK;

  /* Extract Channel Number */
  ChannelNo = (uint8)(LocalIcuChanPtr->IcuProperties.AssignedHwUnitNumber) &
              ICU_LOWER_NIBBLE;
  /* Edge to be configured */
  LActiveEdge = (uint32)LocalIcuChanPtr->IcuProperties.DefaultStartEdge + 1U;
  /* Fill the User selected port */
  Gpt12Ptr.PortInSelReg = LocalIcuChanPtr->IcuProperties.PinSelection;
  /* Reset the Timer counter */
  Gpt12Ptr.TimerCntReg = 0U;
  /* Select the timer run configuration depending on ICU channel mode */
  switch((uint32)LocalIcuChanPtr->IcuProperties.MeasurementMode)
  {
    case (uint32)ICU_17_TIMERIP_MODE_EDGE_COUNTER:
      /* GPT12 Timer should not be started after init for edge counting mode */
      Gpt12Ptr.TimerCtrlReg = ((uint32)ICU_GPT12_COUNTER_MODE) | LActiveEdge;
      break;
    case (uint32)ICU_17_TIMERIP_MODE_INCREMENTAL_INTERFACE:
      /* GPT12 Timer should not be started after init for edge counting mode */
      Gpt12Ptr.TimerCtrlReg = ((uint32)ICU_GPT12_INC_INTERFACE_MODE) | 
                                                              ICU_GPT12_UDE_SET;
      Gpt12Ptr.TimerCtrlReg |= 
                             LocalIcuChanPtr->IcuProperties.MeasurementProperty;
      break;
    default: /* ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT */
      /* Start the GPT12 timer after Init for edge detect */
      Gpt12Ptr.TimerCtrlReg = \
          (uint32)((uint32)ICU_GPT12_TX_RUN << ICU_GPT12_TXCON_TXR_POS);
      Gpt12Ptr.TimerCtrlReg |= ((uint32)ICU_GPT12_COUNTER_MODE) | LActiveEdge;
      break;
  }

  /* Fill the channel number */
  Gpt12Ptr.TimerId = ChannelNo;
  ...
  [/cover] */
  ErrorFlag = Mcu_17_Gpt12_TimerInitCheck(&Gpt12Ptr);
  ...
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  an MCU API which simply access but does not pass it further or store is global
  variables. */
  Mcu_17_Ccu6_TimerInit(&Ccu6CfgPtr);

  /* Enable the T12 clock */
  /* [cover parentID={C3A03FA4-5B42-45f8-978B-5AF462489AEA}]
  Enable Timer clock
  [/cover] */
  Mcu_17_Ccu6_TimerShadowTransfer(Ccu6CfgPtr.TimerId);
  /* [cover parentID={90D4B163-8E51-4fa9-A42A-01271FB8C145}]
  Start Timer
  [/cover] */
  Mcu_17_Ccu6_TimerStart(Ccu6CfgPtr.TimerId);
}
#endif
/* #if (ICU_17_TIMERIP_CCU6_USED == STD_ON) */
  ...
  const Icu_17_TimerIp_ChannelConfigType *const LocalIcuChanPtr)
{
  ...
  switch((uint32)LocalIcuChanPtr->IcuProperties.MeasurementMode)
  {
    /* [cover parentID={92027A27-9C65-4802-ACD5-0A3ED70A6201}]
    Is channel in Edge counting?
    [/cover] */
    case (uint32)ICU_17_TIMERIP_MODE_EDGE_COUNTER:
      /* [cover parentID={59BE7CB4-0675-4be9-8B3D-FB9F033F61B0}]
      Set GPT12 timer in counter mode and default active edge.
      [/cover] */
      /* GPT12 Timer should not be started after init for edge counting mode */
      Gpt12Ptr.TimerCtrlReg = ((uint32)ICU_GPT12_COUNTER_MODE) | LActiveEdge;
      break;
    /* [cover parentID={B0B95548-DFE7-45e7-A956-012E955FB76A}]
    Is channel in Incremental Interface mode?
    [/cover] */
    case (uint32)ICU_17_TIMERIP_MODE_INCREMENTAL_INTERFACE:
      /* [cover parentID={419BE63B-1612-4800-B29B-3635FD85ED71}]
      Set GPT12 timer in incremental interface mode and configured counting 
      mechanism.
      [/cover] */
      /* GPT12 Timer should not be started after init for edge counting mode */
      Gpt12Ptr.TimerCtrlReg = ((uint32)ICU_GPT12_INC_INTERFACE_MODE) | 
      ...
      break;
    default: /* ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT */
      /* [cover parentID={2973DAC2-B61B-43a8-9CFD-324E39C9D1DC}]
      Set GPT12 timer in counter mode and default active edge. Start the timer.
      [/cover] */
      /* Start the GPT12 timer after Init for edge detect */
      Gpt12Ptr.TimerCtrlReg = \
          (uint32)((uint32)ICU_GPT12_TX_RUN << ICU_GPT12_TXCON_TXR_POS);
  ...
  an MCU API which simply access but does not pass it further or store is global
  variables. */
  Mcu_17_Gpt12_TimerInit(&Gpt12Ptr);
  ...
  if (ErrorFlag != ICU_ERROR_FOUND)
  {
    ...
    if (ErrorFlag != ICU_ERROR_FOUND)
    {
      ...
      /* [cover parentID={5D1BC239-42D7-46b3-84B1-B4F5D77B0827}]
       Is channel measurement mode edge detection or timestamp?
       [/cover] */
      if (!((LocalIcuChanPtr->IcuProperties.MeasurementMode
             == (uint32) ICU_17_TIMERIP_MODE_SIGNAL_EDGE_DETECT)
            || (LocalIcuChanPtr->IcuProperties.MeasurementMode
                == (uint32) ICU_17_TIMERIP_MODE_TIMESTAMP)))
      {
        ErrorFlag = ICU_ERROR_FOUND;
                            ...
                            const uint8 Value)
{
   ...
   * argument for the API discards volatile keyword. No side effects foreseen
   * by violating this MISRA rule*/
  Ptr = (sint32*)(&(MODULE_SCU.IGCR[EruIgcrUsed / ICU_ERU_CHNL_PAIR_COUNT].U));
  /* [cover parentID={8B697CD7-69E8-4d39-A6B6-C868C1DFEAEB}]
    Hardware register access.
  [/cover] */
  ...
  const uint8 SignalMode)
{
  ...
  TempRegVal |= (uint32)(((uint32)(((uint32)((uint32)ConfigEdge) & \
             ...
             (uint32)(ICU_TIM_CTRL_GPRxSEL_MASK << ICU_TIM_CTRL_GPR0SEL_POS) | \
             (uint32)(ICU_TIM_CTRL_GPRxSEL_MASK << ICU_TIM_CTRL_GPR1SEL_POS)) ;
...
**   const Icu_17_TimerIp_ChannelType Channel, const uint32 StatusFlags)      **
**                                                                            **
        ...
        == (uint8) (LocalIcuChanPtr->IcuProperties.MeasurementProperty))
    {
          ...
          == ChannelDataPtr->TimeCaptureParas.Size)
      {
        ...
        #endif
        {
          #if(ICU_17_TIMERIP_CCU6_USED == STD_ON)
          /* MISRA2012_RULE_2_2_JUSTIFICATION: No side effects foreseen
           * by violating this MISRA rule, as the function performs action using
           * assembly instructions */
