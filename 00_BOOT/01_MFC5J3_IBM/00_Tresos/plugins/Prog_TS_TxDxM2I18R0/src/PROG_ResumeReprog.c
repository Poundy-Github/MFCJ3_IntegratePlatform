...
#if PROG_RESUM_REPROG == STD_ON
tUdsStatus PROG_GetResumeAddress(u16 *puwLen, u8 *aubUdsData)
{
    u8 LogicalBlockId;
    u32 ResumeAddress;
    ...
    if((puwLen != NULL_PTR) && (aubUdsData != NULL_PTR))
    {
        ...
        *puwLen = (PROG_BLOCK_NB * PROG_RESUM_ADDR_SIZE) + 4U;
        eUdsStatus = UDS_ACK;
    }
    else
    {
...
{
#if PROG_RESUM_REPROG == STD_ON
    if (m_stSegList.ubSegListCrcIndex < (m_stSegList.ubSegNbr-1))
    {
