...
#endif /* if (PDUR_DEV_ERROR_DETECT == STD_ON) */
   {
      ...
      *AvailableDataPtr = (PduLengthType)PduR_CalcAvailableDataSize(SessionIndex);
      ...
      else
      {
         ...
         /* check if Transmit call is pending */
         ...
         {
            /* This LoTp shall wait until all other calls to LoTp_Transmit are completed
             * This eliminates any race conditions caused by PduR_GateTpCopyTxData preempting
             * PduR_GateTpCopyRxData */

            RetVal = BUFREQ_E_BUSY;
         }
         /* all Transmit calls are completed (either successfully or failed) */
         /* direct gateway */
