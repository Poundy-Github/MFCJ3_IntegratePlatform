...
#if( (DMA_INIT_API_MODE     != DMA_MCAL_SUPERVISORMODE)||\
...
#if ( DMA_SW_MAJOR_VERSION != 10U )
  #error "DMA_SW_MAJOR_VERSION does not match."
#endif
#if ( DMA_SW_MINOR_VERSION != 30U )
...
/* ---- RUNTIME MACROS ---- */
  ...
  /* Supervisory mode registers */
  /* MISRA2012_RULE_4_9_JUSTIFICATION: Function like macro
   * 'DMA_RUNTIME_SETBITATOMIC'
   * defined for User mode support in code.
   * No side effects foreseen by violating this MISRA rule. */
  #define  DMA_RUNTIME_SETBITATOMIC(DataPtr,BitPos,BitLen,Data) \
               Mcal_SetBitAtomic(DataPtr,BitPos,BitLen,Data)
 ...
 * No side effects foreseen by violating this MISRA rule. */
  #define  DMA_RUNTIME_SETBITATOMIC(DataPtr,BitPos,BitLen,Data) \
               MCAL_LIB_SETBITATOMIC(DataPtr,BitPos,BitLen,Data)
 ...
 * 'Dma_CoreTosMap' before #include memap.h - Accepted deviation in AUTOSAR */
#include "Dma_MemMap.h"
...
#include "Dma_MemMap.h"
   ...
   * memap.h - Accepted deviation in AUTOSAR */
  #include "Dma_MemMap.h"
...
(
  const Ifx_DMA_ME_ERRSR lMeErrReg
...
)
{
  ...
  #endif
  {
        ...
        (lTsrReg.B.HLTACK == 0x00U)     )
    {
      ...
      DMA_RUNTIME_SETBITATOMIC( (&(MODULE_DMA.TSR[Channel].U)),
                                IFX_DMA_TSR_ECH_OFF,
                                0x01,
                                DMA_BIT_SET );
    }
        ...
        (lTsrReg.B.HLTACK   == 0x00U) )
    {
      ...
      if(Event == DMA_EVENT_CH_TRANSFER_COMPLETE)
      {
        ...
        DMA_RUNTIME_SETBITATOMIC(
                ...
                (uint8)(IFX_DMA_ME_ADICR_INTCT_OFF+1u),
                0x01,
                DMA_BIT_SET );
      }
      else if(Event == DMA_EVENT_CH_BUFFER_WRAP_SOURCE)
  ...
  #endif
  {
        ...
        (lTsrReg.B.HLTACK   == 0x00U) )
    {
      if(Event == DMA_EVENT_CH_TRANSFER_COMPLETE)
      {
        ...
        DMA_RUNTIME_SETBITATOMIC(
                (&(MODULE_DMA.CH[Channel].ADICR.U)),
                (uint8)(IFX_DMA_CH_ADICR_INTCT_OFF+1u),
                0x01,
                DMA_BIT_CLEAR );
      }
      else if(Event == DMA_EVENT_CH_BUFFER_WRAP_SOURCE)
        ...
        (Dma_kConfigPtr->DmaCoreConfigPtr[lCoreId] != NULL_PTR) )
    {
      ...
      if(lChIndex != DMA_CHANNEL_MAP_RESET_VALUE)
      {
        ...
        if(lChNotify != NULL_PTR)
        {
          ...
          /* Check for channel transfer Interrupt */
          if(lChcsrReg.B.ICH == DMA_BIT_SET)
          {
          ...
          if(lChcsrReg.B.WRPD == DMA_BIT_SET)
          {
            ...
            /* Clear wrap buffer status bit */
            lChcsrReg.B.CWRP = DMA_BIT_SET;
          ...
          {
            /* Write the register values to the register */
            MODULE_DMA.CH[Channel].CHCSR.U = lChcsrReg.U;
          }
      ...
      if(lChIndex != DMA_CHANNEL_MAP_RESET_VALUE)
      {
        ...
        if(lMeNotify != NULL_PTR)
        {
              ...
              (lErrorSource == DMA_MOVE_ENGINE_1) )
          {
            ...
            /* Clear the ME errors */
            lMeClrReg->U = DMA_CLEAR_ERROR_MASK;
          }
          else
          {
            if(MODULE_DMA.TSR[lChannelId].B.TRL == DMA_BIT_SET)
            {
              ...
              MODULE_DMA.TSR[lChannelId].U = lTsrReg.U;
...
)
{
  ...
  Xor the RDCRC register content against configured data CRC
  [/cover] */
...
)
{
  ...
  Det_ReportError( DMA_MODULE_ID, DMA_MODULE_INSTANCE, ApiId, ErrorId);
  #endif
...
**                                                                            **
** Syntax           : void Dma_lEvaluateMEError(Ifx_DMA_ME_ERRSR lMeErrReg)   **
**                                                                            **
...
** Reentrancy       : Reentrant                                               **
**                                                                            **
** Parameters (in)  : lMeErrReg - The pointer to the ME register              **
**                                                                            **
** Parameters (out) : None                                                    **
**                                                                            **
...
)
{
  ...
  if(lMeErrReg.B.SER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.DER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.RAMER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.SLLER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.DLLER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.SPBER == DMA_BIT_SET)
  {
  ...
  if(lMeErrReg.B.SRIER == DMA_BIT_SET)
  {
