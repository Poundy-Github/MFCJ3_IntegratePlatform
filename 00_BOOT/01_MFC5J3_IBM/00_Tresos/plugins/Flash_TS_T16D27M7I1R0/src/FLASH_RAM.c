...
tFlashStatus FLASH_RAM_WriteRoutine(tFlashAddress uAddr,u16 uwLen,const u8 FLASH_FAR_POINTER aubData)
{
    ...
    /* CHECK: NOPARSE */
    /* NOCOV-DEFENSIVE*/
    if(tWrite_fptr != NULL_PTR)
    {
        /* Write Operation */
        ulFlsStatus = tWrite_fptr((FlsLoader_AddressType) uAddr,
                (FlsLoader_LengthType) uwLen, (const u8*) aubData);
    }
    else
    {
        ulFlsStatus = FLSLOADER_E_NOT_OK;
    }
    /* CHECK: PARSE */
...
tFlashStatus FLASH_RAM_EraseRoutine(u16 ulSectorNo)
{
    ...
    /* CHECK: NOPARSE */
    /* NOCOV-DEFENSIVE*/
    if(tErase_fptr != NULL_PTR)
    {
        ulFlsStatus = tErase_fptr((FlsLoader_AddressType) uAddr,
