...
#if ( SMU_SW_MAJOR_VERSION != 10U )
  #error "SMU_SW_MAJOR_VERSION does not match."
#endif

#if ( SMU_SW_MINOR_VERSION != 30U )
                             ...
                             const uint8 ErrorId)
{
  ...
  [/cover] */
  Det_ReportError(SMU_MODULE_ID, SMU_MODULE_INSTANCE, \
                                ...
                                const Smu_AlarmIdType AlarmPos)
{
  ...
  else if((uint8)AlarmGroup == (uint8)SMU_ALARM_GROUP20)
  {
    ...
    SMU_LIB_RUNTIME_WRITESAFETYENDINITPROTREG(&MODULE_PMS.CMD_STDBY, \
        (SMU_STDBY_BIT_PROT | SMU_STDBY_ASCE_COMMAND));
    /*Clear the alarm configuration register for the requested alarm group
    20 position. */
                               ...
                               const Smu_FSPActionType FSPAction)
{
  ...
  if((uint8)AlarmGroup < SMU_CORE_TOTAL_ALARM_GROUPS)
  {
    ...
    input parameter*/
    /* [cover parentID={3879E998-D1E6-4930-8E4C-05BAEC6D62FE}]
    Is the required alarm position Alarm Position 2 or higher ?
    [/cover] */
    if((uint8)AlarmPos >= SMU_ALARM_POS2)
    {
      /* [cover parentID={B6576F5B-2DB9-4a3c-958F-4631A45B0F52}]
      Extract Configuration Group 0 as per mask from input parameter
      for alarm group greater or equal to 2
      [/cover] */
      AlarmGroupCF0 = (uint32)(((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG0_MASK)<< (uint32)AlarmPos);

      AlarmGroupCF1 = (uint32)(((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG1_MASK) << ((uint32)AlarmPos - \
            (uint32)SMU_ALARM_POS1));

      AlarmGroupCF2 = (uint32)(((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG2_MASK)<< ((uint32)AlarmPos - \
            (uint32)SMU_ALARM_POS2));
    }
    /* [cover parentID={6882B813-322C-4dd8-A327-19AB66EADA2E}]
    Is the required alarm position Alarm Position 1 ?
    [/cover] */
    else if((uint8)AlarmPos == SMU_ALARM_POS1)
    {
      /* [cover parentID={1EEC00EC-828F-4a1e-A1B7-DE7C392CC12B}]
      Extract Configuration Group 0 as per mask from input parameter for
      alarm group equal to 1
      [/cover] */
      AlarmGroupCF0 = (uint32)(((uint32)AlarmAction & \
            (uint32) SMU_ALMGRP_CFG0_MASK) << (uint32)AlarmPos);

      AlarmGroupCF1 = (uint32)((uint32)AlarmAction & (uint32) \
            SMU_ALMGRP_CFG1_MASK);
      AlarmGroupCF2 = (uint32)(((uint32)AlarmAction & (uint32) \
            SMU_ALMGRP_CFG2_MASK) >> (uint32)SMU_ALARM_POS1);
    }
    /* [cover parentID={27110D7B-1C67-4349-B1F4-0ADB2F6DC520}]
    Extract Configuration Group 0 as per mask from input parameter for
    alarm group equal to 0
    [/cover] */
    else /* for (AlarmPos == 0U)*/
    {
      /* [cover parentID={27110D7B-1C67-4349-B1F4-0ADB2F6DC520}]
      Extract Configuration Group 0 as per mask from input parameter for alarm 
      group equal to 0
      [/cover] */
      AlarmGroupCF0 = (uint32)((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG0_MASK);
      AlarmGroupCF1 = (uint32)(((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG1_MASK) >> (uint32)SMU_ALARM_POS1);
      AlarmGroupCF2 = (uint32)(((uint32)AlarmAction & \
            (uint32)SMU_ALMGRP_CFG2_MASK) >> SMU_ALARM_POS2);
    }
  ...
  else if(AlarmGroup == SMU_ALARM_GROUP20)
  {
    ...
    [/cover] */
    SMU_LIB_RUNTIME_WRITESAFETYENDINITPROTREG(&MODULE_PMS.AGFSP_STDBY0, \
                                              ((SMU_STDBY_BIT_PROT | \
                                              (FSPAction << AlarmPos)) | \
                                               PMS_AGFSP_STDBY0.U));
    RetVal = E_OK;
  ...
  else
  {
    ...
    #endif
    {
      ...
      SMU_LIB_RUNTIME_WRITESAFETYENDINITPROTREG( \
          ...
          (FSPAction << AlarmPos)) | PMS_AGFSP_STDBY1.U));
      RetVal = E_OK;
    }
  }
  #endif
                              ...
                              Smu_FSPActionType * const FSPAction)
{
  ...
  if((uint8)AlarmGroup < SMU_CORE_TOTAL_ALARM_GROUPS)
  {
    ...
    /* [cover parentID={1FE6E077-C72E-4d65-97B7-5CB8364ED0DA}]
    Is the required alarm position Alarm Position 2 or higher ?
    [/cover] */
    if((uint32)AlarmPos >= SMU_ALARM_POS2)
    {
      /* [cover parentID={0F3EBF11-7258-4448-A572-A035EA943164}]
      Read from configuration group 0 as per mask for alarm position
      ...
      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      pointers of different object types due to SFR access. */
      AlarmGroupCF1 = (uint32) \
            (((uint32)(SMU_CORE_ALARM_GROUP->CfgReg[(AlarmGroupCFIndex + \
            (uint32)SMU_ALARM_POS1)].U)>>((uint32)AlarmPos- \
            (uint32)SMU_ALARM_POS1))& (uint32)SMU_ALMGRP_CFG1_MASK);

      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      pointers of different object types due to SFR access. */
      AlarmGroupCF2 = (uint32)(((uint32)(SMU_CORE_ALARM_GROUP->CfgReg \
            [(AlarmGroupCFIndex + (uint32)SMU_ALARM_POS2)].U)>> \
            ((uint32)AlarmPos-(uint32)SMU_ALARM_POS2))& \
            (uint32)SMU_ALMGRP_CFG2_MASK);

    }
    /* [cover parentID={4BF74092-D450-4874-A60E-32675B0DEA83}]
    Is the required alarm position Alarm Position 1 ?
    [/cover] */
    else if((uint32)AlarmPos == SMU_ALARM_POS1)
    {
      /* [cover parentID={974D3B47-9C91-49ae-A9A7-C39DE59E42CE}]
      Read from configuration group 0 as per mask for alarm position
      equal to 1
      [/cover] */
      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      ...
      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      pointers of different object types due to SFR access. */
      AlarmGroupCF1 = (uint32)((uint32) \
            (SMU_CORE_ALARM_GROUP->CfgReg[(AlarmGroupCFIndex + \
            (uint32)SMU_ALARM_POS1)].U) & (uint32)SMU_ALMGRP_CFG1_MASK );

      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      pointers of different object types due to SFR access. */
      AlarmGroupCF2 = (uint32)(((uint32) \
            (SMU_CORE_ALARM_GROUP->CfgReg[(AlarmGroupCFIndex + \
            (uint32)SMU_ALARM_POS2)].U) << (uint32)SMU_ALARM_POS1) \
            & (uint32)SMU_ALMGRP_CFG2_MASK );
    }
    else/* for (AlarmPos == 0U)*/
    {
      /* [cover parentID={987E9BE5-C10B-4776-8BAE-CCB093A335B3}]
      Read from configuration group 0 as per mask for alarm position
      equal to 0
      [/cover] */
      /* MISRA2012_RULE_11_3_JUSTIFICATION: Conversion between
      ...
      AlarmGroupCF1 = (uint32)(((uint32) \
            (SMU_CORE_ALARM_GROUP->CfgReg[(AlarmGroupCFIndex + \
            (uint32)SMU_ALARM_POS1)].U) << (uint32)SMU_ALARM_POS1) \
      ...
      AlarmGroupCF2 = (uint32)(((uint32) \
            (SMU_CORE_ALARM_GROUP->CfgReg[(AlarmGroupCFIndex+ \
            (uint32)SMU_ALARM_POS2)].U)<<(uint32)SMU_ALARM_POS2)& \
            (uint32)SMU_ALMGRP_CFG2_MASK);
    }
...
)
{
  ...
  #endif
  {
    ...
    if(AlarmStatusReadback != SMU_CLEAR_ALARM_STATUS_READBACK)
    {
      ...
      DEM notification for clearing alarm status failure or success is
      enabled
      [/cover] */
      #if (SMU_CLEAR_ALARM_STATUS_DEM_NOTIF == \
      SMU_ENABLE_DEM_REPORT)
      /* Report failure as DEM event status*/
      /* [cover parentID={433D1BD1-EBB3-44fa-A167-403123BD967D}]
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_CLEAR_ALARM_STATUS_FAILURE,
    ...
    else
    {
      ...
      #if (SMU_CLEAR_ALARM_STATUS_DEM_NOTIF == \
      SMU_ENABLE_DEM_REPORT)
      /* Report success as DEM event status*/
...
)
{
  ...
  #endif
  {
    ...
    if(AlarmStatusReadback != SMU_SET_ALARM_STATUS_READBACK)
    {
      ...
      DEM notification for setting alarm status failure is enabled
      [/cover] */
      ...
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_SET_ALARM_STATUS_FAILURE,
                            DEM_EVENT_STATUS_FAILED);
      #endif
    }
    else
    {
      ...
      DEM notification for setting alarm status failure or
      success is enabled
      [/cover] */
      #if (SMU_SET_ALARM_STATUS_DEM_NOTIF == \
      SMU_ENABLE_DEM_REPORT)
...
)
{
  ...
  #endif
  {
    ...
    if(AlarmGroup == SMU_ALARM_GROUP20)
    {
      ...
      Read the alarm status from Alarm group 20
      [/cover] */
      *AlarmStatus = (uint32)(PMS_AG_STDBY0.U);
      RetVal = E_OK;
    }
  ...
  #endif
  {
       ...
       (SmuEFRST == (uint32)SMU_EFRST_ENABLE)))
    {
      ...
      if(CommandStatus != SMU_COMMAND_SUCCESS)
      {
        ...
        DEM notification for FSP release failure or success is enabled
        [/cover] */
        ...
        Report failure as DEM event status
        [/cover] */
        Dem_ReportErrorStatus(SMU_E_RELEASE_FSP_FAILURE, \
                              DEM_EVENT_STATUS_FAILED);
        #endif
      }
      else
...
Std_ReturnType Smu_ActivateFSP(void)
{
  ...
  #endif
  {
    ...
    if(CommandStatus != SMU_COMMAND_SUCCESS)
    {
      ...
      DEM notification for FSP activation failure or success is enabled
      [/cover] */
      #if (SMU_ACTIVATE_FSP_FAILURE_DEM_NOTIF == SMU_ENABLE_DEM_REPORT)
      /* Raise failure DEM, when activate FSP fails*/
      /* [cover parentID={ADE66FCE-FAFB-493b-BC41-21261F1C93ED}]
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_ACTIVATE_FSP_FAILURE,
                            DEM_EVENT_STATUS_FAILED);
      #endif
    }
    else
...
Std_ReturnType Smu_RTStop(const uint8 TimerNum )
{
  ...
  #endif
  {
    ...
    if(CommandStatus != SMU_COMMAND_SUCCESS)
    {
      ...
      DEM notification for stop recovery timer command failure or
      success is enabled
      [/cover] */
      /* [cover parentID={3D1FDAEF-C113-4ebd-9D5F-34D4D373547A}]
      Is DEM notification for stop recovery timer command failure or
      success enabled?
      [/cover] */
      ...
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_RT_STOP_FAILURE, \
                            DEM_EVENT_STATUS_FAILED);
      #endif
    }
    else
...
*******************************************************************************/
/* MISRA2012_RULE_8_7_JUSTIFICATION:Smu_GetSmuState is an exported API and will 
be called by upper layers, hence needs external linkage */
...
Std_ReturnType Smu_ActivateRunState(const uint32 Cmd)
{
  ...
  #endif
  {
    ...
    else if (Cmd != (uint32)SMU_RUN_COMMAND)
    {
      ...
      /* Activate FSP by Writting SMU_ActivateFSP(0) in
          CMD register */
      SMU_SFR_RUNTIME_WRITE32(&SMU_CMD.U, (uint32)SMU_ACTIVATEFSP_COMMAND);
    ...
    else
    {
      ...
      if(CommandStatus != SMU_COMMAND_SUCCESS)
      {
        ...
        DEM notification for activating run state failure is enabled
        [/cover] */
        ...
        Report failure as DEM event status
        [/cover] */
        Dem_ReportErrorStatus(SMU_E_ACTIVATE_RUN_STATE_FAILURE,
                              DEM_EVENT_STATUS_FAILED);
        #endif
      }
      else
      {
        ...
        DEM notification for activating run state failure or success
        is enabled
        [/cover] */
        #if (SMU_ACTIVATE_RUN_STATE_FAILURE_DEM_NOTIF == \
        SMU_ENABLE_DEM_REPORT)
...
Std_ReturnType Smu_ActivatePES(void)
{
  ...
  #endif
  {
    ...
    if(CommandStatus != SMU_COMMAND_SUCCESS)
    {
      ...
      DEM notification for activating PES failure or success is enabled
      [/cover] */
      #if (SMU_ACTIVATE_PES_FAILURE_DEM_NOTIF == SMU_ENABLE_DEM_REPORT)
      /* Raise failure DEM, when activate PES fails*/
      /* [cover parentID={57A96D3C-44BB-46ae-8BD6-201B345AB9BA}]
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_ACTIVATE_PES_FAILURE, \
                            DEM_EVENT_STATUS_FAILED);
      #endif
    }
    else
    {
      ...
      [/cover] */
      #if (SMU_ACTIVATE_PES_FAILURE_DEM_NOTIF == SMU_ENABLE_DEM_REPORT)
      /* Raise failure DEM, when activate PES is passed*/
  ...
  #endif
  {
    ...
    if (SmuState == SMU_START_STATE)
    {
      ...
      if(CommandStatus != SMU_COMMAND_SUCCESS)
      {
        ...
        DEM notification for core alive failure is enabled
        [/cover] */
        ...
        Report failure as DEM event status
        [/cover] */
        Dem_ReportErrorStatus(SMU_E_CORE_ALIVE_FAILURE, \
                              DEM_EVENT_STATUS_FAILED);
        #endif
      ...
      else
      {
        ...
        DEM notification for core alive failure or success is enabled
        [/cover] */
        #if (SMU_CORE_ALIVE_FAILURE_DEM_NOTIF == \
        SMU_ENABLE_DEM_REPORT)
        /* Raise failure DEM, when activate PES fails*/
...
)
{
  ...
  if(AlarmExecStatus != NULL_PTR)
  {
    ...
    if(RetVal == E_OK)
    {
      ...
      Check if requested Alarm execution Status is in valid range
      [/cover] */
      ...
      else
      {
        ...
        Check if requested Alarm execution Status is reserved
        [/cover] */
        ...
        Is the requested Alarm execution Status reserved?
        [/cover] */
        if(AlarmRes == SMU_ALARM_INVALID)
        {
  ...
  #endif
  {
    /*Read the alarm execution staus of the requested staus bit*/
    /* [cover parentID={9F870C5A-5499-4c4b-B3FD-5AB72A93871E}]
    Extract Alarm Execution Status
    [/cover] */
...
)
{
  ...
  if(RetVal == E_OK)
  {
    ...
    Is the alarm exceution status request within a valid range?
    [/cover] */
    if(AlarmExecStatusReq > SMU_ALMEXEC_MSK)
    {
  ...
  #endif
  {
    /*Read the result of the request staus bit*/
    /* [cover parentID={C7DE04ED-9E3E-4505-BDF8-522B457BEBC3}]
    Read the result of the request status bit
    [/cover] */
    /* [cover parentID={0D20AD6F-7260-4c0d-90C9-531A056367E4}][/cover] */
    StatusClearRes = (((uint32)SMU_ALARM_VALID << \
                       (uint32)AlarmExecStatusReq));
...
)
{
  ...
  #endif
  {
    ...
    /* [cover parentID={BB923ED4-973C-4afe-8270-CBDD00DA8116}]
      Start SFF tests
    [/cover] */
    ...
    if (Timeout >= SFF_RM_TIMEOUT)
    {
      ...
      DEM notification for SFF test failure is enabled
      [/cover] */
      #if(SMU_SFF_TEST_FAILURE_DEM_NOTIF == SMU_ENABLE_DEM_REPORT)
      /* Raise failure DEM, when clear alarm status fails*/
      /* [cover parentID={0E00500E-341E-46a8-B2A8-66F2F0FD41FB}]
      Report failure as DEM event status
      [/cover] */
      Dem_ReportErrorStatus(SMU_E_SFF_TEST_FAILURE, \
                            DEM_EVENT_STATUS_FAILED);
      #endif
    }
    else
    {
      ...
      DEM notification for SFF test failure or success is enabled
      [/cover] */
      #if (SMU_SFF_TEST_FAILURE_DEM_NOTIF == SMU_ENABLE_DEM_REPORT)
      /* Raise failure DEM, when clear alarm status fails*/
