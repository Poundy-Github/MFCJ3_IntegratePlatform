...
#define COM_EB_MULTIGW_INCREASE_SRC_COUNT(IPduPtr)                                    \
   do{                                                                                \
     ...
     P2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) MultiGWSrcStatusptr =                    \
...
(
   ...
   P2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) SrcDataPtr
#if (COM_MULTI_ROUTE_MAIN_ENABLE == STD_ON)
...
(
   ...
   P2CONST(void, AUTOMATIC, COM_VAR_NOINIT) SignalDataPtr,
...
)
{
   ...
   for (ii = 0U; (ii < Com_gConfigPtr->ComSigGWTxSignalGroupQueueMax) && (exit_flag == 1U); ii++)
   {
      ...
      CONSTP2VAR(ComSignalGroupRefType, AUTOMATIC, COM_VAR_NOINIT) TxSignalGroupQueueBase =
...
)
{
   ...
   P2VAR(void, AUTOMATIC, COM_VAR_NOINIT) SignalDataPtr = &valuint8;
...
)
{
...
#endif /* (COM_RX_DYN_LENGTH_IPDU_ENABLE == STD_ON) && (COM_TX_DYN_LENGTH_IPDU_ENABLE == STD_ON) */
   {
      ...
      CONSTP2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) DestDataPtr =
   ...
   if (Com_InitStatus == COM_EB_INIT)
   {
               ...
               sigGWRxPduRef++)
      {
         ...
         if ( IPduPtr->ComMainFunctionsRx_Idx == ComMainFunctionsRx_Idx )
         {
            ...
            CONSTP2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) MultiGWSrcStatusptr =
                     &Com_gDataMemPtr[COM_EB_MULTIGW_GET_SRC_STATUS_REF(IPduPtr)
                                      + ComMainFunctionsTx_Idx];

            CONSTP2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) MultiGWDestStatusptr =
         ...
         if ( IPduPtr->ComMainFunctionsRx_Idx == ComMainFunctionsRx_Idx )
         {
                     ...
                     COM_RX_IPDU_SIG_GW_FLAG_BIT(IPduPtr)) )
            {
                        ...
                        COM_RX_IPDU_SIG_GW_FLAG_BIT(IPduPtr));
               {
                  ...
                  P2CONST(uint8, AUTOMATIC, COM_VAR_NOINIT) pdudataptr =
                           &Com_gDataMemPtr[COM_EB_GET_SRCVALUE_REF(IPduPtr)];

                  P2VAR(uint8, AUTOMATIC, COM_VAR_NOINIT) MultiGWpdudataptr =
