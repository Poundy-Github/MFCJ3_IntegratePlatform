...
void PROG_GetPublicKey(Csm_AsymPublicKeyType* pstPublicKey)
{
    ...
    pstPublicKey->data[0U] = (u8)(ulPublicExponent >> 24); /* Exponent */
    pstPublicKey->data[1U] = (u8)(ulPublicExponent >> 16); /* Exponent */
    pstPublicKey->data[2U] = (u8)(ulPublicExponent >> 8); /* Exponent */
    ...
    for(uwIdx = 0U; /* Modulus */
        uwIdx < pstPublicKey->data[(PROG_KEY_LENGTH/sizeof(u32)) + 1];
        uwIdx++)
    {
...
Csm_ReturnType PROG_CsmSetPreConditions(const u8 * ubKeyPtr, u8 ubProgCsmJobTabId)
{
    ...
    if((m_astProgCsmJobConf[ubProgCsmJobTabId].eAllowKeySet == PROG_TRUE) && \
            (ubPtr2Key != NULL_PTR) && (eCsmStatus == E_OK))
    {
