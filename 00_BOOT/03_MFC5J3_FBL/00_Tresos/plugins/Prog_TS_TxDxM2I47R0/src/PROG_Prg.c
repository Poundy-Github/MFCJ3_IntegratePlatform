...
/* end of PROG_ERASE_TYPE == PROG_ERASE_BY_BLOCK_ID */
    ...
    m_stSegList.ubSegListCrcIndex = 0;
        ...
        if (!((m_stDiagMessageInfo.pubTxData[0] == UDS_SRV_RC) && (m_stDiagMessageInfo.pubTxData[2] == 0xFF) && (m_stDiagMessageInfo.pubTxData[3] == 0x01)))
#endif
        {
    ...
    if (PROG_E_OK == eProgStatus)
    {
        ...
        if (PROG_E_OK == eProgStatus)
        {
            ...
            eProgStatus = PROG_GetSegmentByAddress(stRDParam.ulStartAddress,
                    ...
                    &ubLocalSegmentId);
/*check running area whether is same to download area,if no, return E_OK*/
            if (PROG_E_OK == eProgStatus)
            {
                eProgStatus = PROG_CustomSegmentCheck(ubLocalSegmentId);
            }
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
            if (PROG_E_OK == eProgStatus)
            {
...
//#elif (PROG_VERIFICATION_ON_THE_FLY == STD_OFF)
                ...
                //deleted by jianquan PROG_EVENT_FINISHED will be emit by PROG_CRC
                // PROG_HsmEmit(&PROG_HsmScPROG, PROG_HSM_PROG_EV_PROG_EVENT_FINISHED);
...
void PROG_RTE(void)
{
    ...
    if(eProgStatus == PROG_E_OK)
    {
        ...
        // m_stDiagMessageInfo.ulLength = 3;
...
/*check running area whether is same to download area,if no, return E_OK*/
            if (PROG_E_OK == eProgStatus)
            {
                eProgStatus = PROG_CustomSegmentCheck(pstSegment->ubSegmentId);
            }
#if (PROG_IMPL_VARIANT == PROG_IMPL_11)
    if (PROG_E_OK == eProgStatus)
    {
...
void PROG_Entry_BLU_Resume(void)
{
    ...
    (void)EB_SimulateRxRequest(stBLUContextRestore.stBLUCheckMemoryRequestInfo.ulLength,
                               stBLUContextRestore.stBLUCheckMemoryRequestInfo.aubData, TRUE);//FALSE
