...
#else
                /* here, for erasing case, length unit is one sector */
                eFlashStatus = FLASH_Erase(uMemAddress, ulLength);
...
#elif (PROG_FLASHWRAPPER_ENABLED == STD_ON)
                eFlashStatus = FLASH_WriteBuffer(ulLocalMemAddress, (u32)((u32)ulLength & 0xFFFFFFFFU), paubDataBuffer);
#endif
        }
#if (PROG_REPROGMEMM_ENABLED == STD_ON)
      if(DCM_E_OK == RetVal)
...
)
{
    ...
    switch (eOperationType)
    {
...
#else
            eFlashStatus = FLASH_Ext_Erase(uLocalMemAddress,ulLength);
#endif /* PROG_SLICED_ERASING_FOR_NRC78_SENDING == STD_ON */
        ...
        /* PROG_MemWriteBuffer */
        case PROG_MEM_OPERATION_TYPE_WRITE:
            eFlashStatus = FLASH_Ext_WriteBuffer(uLocalMemAddress, (u16)((u16)ulLength & 0xFFFFU), paubDataBuffer);
            if(FLASH_EXT_NO_ERROR == eFlashStatus)
            {
                eProgStatus = PROG_E_OK;
            }
        ...
        /* PROG_MemReaduffer */
        case PROG_MEM_OPERATION_TYPE_READ:
            eFlashStatus = FLASH_Ext_Read(uLocalMemAddress, (u16)((u16)ulLength & 0xFFFFU), paubDataBuffer);
