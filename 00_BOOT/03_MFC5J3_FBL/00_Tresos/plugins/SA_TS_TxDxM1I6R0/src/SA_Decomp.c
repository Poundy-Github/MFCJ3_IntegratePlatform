...
NCS_PRI_VAR u8 m_aubLZSS_window[LZSS_WINDOW_SIZE];
NCS_PRI_VAR tDecompressStateType m_uDecompressState;
NCS_PRI_VAR u16 m_uwWinPos;
NCS_PRI_VAR u8 m_ubOutByte;
NCS_PRI_VAR u16 m_uwMatchPos;
NCS_PRI_VAR u8 m_ubMatchLen;
NCS_PRI_VAR u8 m_ubBitCount;

/* decompression status */
NCS_PRI_VAR tDecompressStateType m_uDecompStatus;

/* Input buffer variable */
NCS_PRI_VAR u8* m_pubInDecompBuf;
NCS_PRI_VAR u16 m_uwInBufLength;
NCS_PRI_VAR u16 m_uwDecompIdx;

/* Output buffer variables */
NCS_PRI_VAR u8 m_aubOutDecompBuf[SA_DECOMP_OUT_BUF_SIZE];
NCS_PRI_VAR u16 m_uwOutBufPosition;
NCS_PRI_VAR u16 m_uwOutBufLength;
...
/*----------------------------------{SA_DecompressData}--------------------------------*/
void SA_DecompressData(u8* pubData, u16 uwDataLength)
{
    ...
    u16* puwDecompressLength)
{
    ...
    if (puwDecompressLength != NULL_PTR)
    {
        if (m_uwOutBufPosition < m_uwOutBufLength)
        {
            /* return the length of available data */
            *puwDecompressLength = (m_uwOutBufLength - m_uwOutBufPosition);
        ...
        {
            /* return the length of available data */
            *puwDecompressLength = m_uwOutBufLength;
            /* return pointer to data to write */
