...
#if (MCU_SW_MINOR_VERSION != 30U)
#error "MCU_SW_MINOR_VERSION does not match."
#endif

#if (MCU_SW_PATCH_VERSION != 1U)
...
#if ((MCU_CCU60_USED == STD_ON) || (MCU_CCU61_USED ==STD_ON))
#define CCU_CLC_MSK               ((uint32)0x0000000AU)
#endif

#if ((MCU_GPT1_USED == STD_ON) || (MCU_GPT2_USED == STD_ON))
#define GPT12_CLC_MSK             ((uint32)0x0000000AU)
#endif
#endif
...
#define MCU_TBU_ENDIS_CH_MASK   ((uint32)0x000000FFU)
#define MCU_TBU_CH_DISABLE_MASK ((uint32)0x00000055U)
#define MCU_TBU_CH_0_2_MASK     ((uint32)0x0000000FU)
...
#define MCU_TBU_CH3_MODE_MASK      ((uint32)0x00000010U)
...
#define MCU_TBU_CH_EN_MSK           (0xFFU)
...
#define MCU_GTM_ATOM_AGC_INT_TRIG_MASK ((uint32)0x0000FFFFU)
#define MCU_GTM_ATOM_AGC_RST_CN0_MASK  ((uint32)0xFFFF0000U)
...
#define MCU_GTM_TOM_TGC_INT_TRIG_MASK ((uint32)0x0000FFFFU)
#define MCU_GTM_TOM_TGC_RST_CN0_MASK  ((uint32)0xFFFF0000U)
#define MCU_TOM_ACT_TB_TRIG_MSK       ((uint32)0x01000000U)
#define MCU_ATOM_ACT_TB_TRIG_MSK      ((uint32)0x01000000U)
...
/* RAR area handling related macros */
#define MCU_RAR_NOT_INSTALLED         ((uint32)0U)
#define MCU_BASE_ADDR_REPAIRED        ((uint32)0xFFFFFFFFU)
#define MCU_RAR_LOC_OFFSET            ((uint32)4U)
#define MCU_RAR_REPAIRED_LOC_OFFSET   ((uint32)12U)
#define MCU_BASE_ADDR_REPAIRED_OFFSET ((uint32)8U)
#endif
...
#if (MCU_NO_OF_STDBY_RAM_BLK != 0U)
LOCAL_INLINE void Mcu_lHandleStandbyRAM(void);
#endif
...
Std_ReturnType Mcu_InitCheck(const Mcu_ConfigType *const ConfigPtr)
{
   ...
   * Is configuration pointer not NULL and MCU driver initialized
   * [/cover] */
  if ((Mcu_DriverState == MCU_DRIVER_INITIALIZED) &&
      (ConfigPtr == Mcu_ConfigPtr))
  {
     ...
     * power management for low power mode standby settings.
     */
    /* [cover parentID={6C83FDC9-4F0E-49f6-BFE1-0DD799637364}]
     * Is low power standby configuration available
     * [/cover] */
    if ((ConfigPtr->McuLowPowerModeCfgPtr->MaxModeEvrcCtrl.McuMode &
         (uint8)(0x1U << MCU_STANDBY)) > (uint8)0x0U)
    {
      SfrVal    = PMS_PMSWCR0.U;
      ConfigVal = (ConfigPtr->McuLowPowerModeCfgPtr->Pmswcr0);
      CmpVal   &= ~(SfrVal ^ ConfigVal);

      SfrVal    = PMS_PMSWCR4.U;
      ConfigVal = (ConfigPtr->McuLowPowerModeCfgPtr->Pmswcr4);
      CmpVal   &= ~(SfrVal ^ ConfigVal);

      SfrVal    = PMS_PMSWCR5.U;
      ConfigVal = ((ConfigPtr->McuLowPowerModeCfgPtr->Pmswcr5) &
                   MCU_PMSWCR5_MASK);
      CmpVal   &= ~(SfrVal ^ ConfigVal);
    }
    /* [cover parentID={2D998127-64C9-4735-AFB0-B3F3F5DF9B31}]
     * Is EVRC sleep mode configuration available
     * [/cover] */
    if ((((uint32)ConfigPtr->McuLowPowerModeCfgPtr->MaxModeEvrcCtrl.McuMode) &
         ((uint32)0x1U << MCU_SLEEP)) > 0x0U)
    {
      SfrVal    = (uint32)SCU_PMTRCSR0.B.LPSLPEN;
      ConfigVal = ConfigPtr->McuLowPowerModeCfgPtr->
                  MaxModeEvrcCtrl.EvrcLowPowerMode;
      CmpVal    &= ~(SfrVal ^ ConfigVal);
    }
...
LOCAL_INLINE void Mcu_lGtmTbuGlobalInit(void)
{
  ...
  GTM_TBU_CHEN.U = MCU_TBU_CH_DISABLE_MASK;
...
LOCAL_INLINE void Mcu_lSetStandbyMode(const uint8 CoreId)
{
  ...
  */
       ...
       (MCU_UNANIMOUS_SEL_SYSTEM_MODE == StandbyModeSel)))
  {
    ...
    {
      /* [cover parentID={63044246-C907-4eb3-A5CC-08E2F2C8C1A9}]
       * If number of Standby RAM block is not zero*/
      #if (MCU_NO_OF_STDBY_RAM_BLK != 0U)
...
/*******************************************************************************
**                                                                            **
** Traceability    : [cover parentID={8B865FCF-A770-40c9-9FC3-B5404B6E45BE}]  **
**                                                                            **
** Syntax          : LOCAL_INLINE void Mcu_lHandleStandbyRAM(void)            **
**                                                                            **
** Description     : This function performs RAM redundancy area handling.     **
**                                                                            **
** Service ID      : None                                                     **
**                                                                            **
** Sync/Async      : Synchronous                                              **
...
LOCAL_INLINE void Mcu_lHandleStandbyRAM(void)
{
  ...
  uint32 RLAAdr;
  /*
    For all configured standby rams:
    1. initialize reserved area pointer to the beginning of the target
       stand-by RAM
    2. read the word pointed by reserved area pointer into
       repaired location address
    3. if repaired location address is zero, no preparation needed.
    4. if repaired location address is 0xFFFFFFFF, move move the reserved area
       pointer 32 Bytes (8 words) forward and read the target word into
       repaired location address
    5. move the reserved area pointer 16 Bytes (4 words) forward and read the
       word from the repaired location address and store at the location
       pointed by reserved area pointer
  */

  for (LoopCtr = MCU_STARTING_INDEX;
       LoopCtr < MCU_NO_OF_STDBY_RAM_BLK; LoopCtr++)
  {
    RARPtr = Mcu_ConfigPtr->McuLowPowerModeCfgPtr->StdbyRamAdr[LoopCtr];
    RLAAdr = *RARPtr;
    if (MCU_RAR_NOT_INSTALLED != RLAAdr)
    {

      if (MCU_BASE_ADDR_REPAIRED == RLAAdr)
      {
        /* MISRA2012_RULE_11_6_JUSTIFICATION: cast from unsigned long to pointer
           is required to read RAR area location address. This action is
           performed only when RAR area is installed. */
        /* MISRA2012_RULE_11_4_JUSTIFICATION: cast from pointer to integer type
           is required to read RAR area location address. This action is
           performed only when RAR area is installed. */
        RLAAdr = *((uint32 *)RARPtr[MCU_BASE_ADDR_REPAIRED_OFFSET]);
        /* MISRA2012_RULE_11_6_JUSTIFICATION: cast from unsigned long to pointer
           is required to restore RAR area from location address. This action is
           performed only when RAR area is installed. */
        /* MISRA2012_RULE_11_4_JUSTIFICATION: cast from pointer to integer type
           is required to restore RAR area from location address. This action is
           performed only when RAR area is installed. */
        RARPtr[MCU_RAR_REPAIRED_LOC_OFFSET] = *((uint32 *)RLAAdr);
      }
      else
      {
        /* MISRA2012_RULE_11_6_JUSTIFICATION: cast from unsigned long to pointer
        ...
        /* MISRA2012_RULE_11_4_JUSTIFICATION: cast from pointer to integer type
           is required to restore RAR area from location address. This action is
           performed only when RAR area is installed. */
        RARPtr[MCU_RAR_LOC_OFFSET] = *((uint32 *)RLAAdr);
      }
    }
  }
}
#endif
/* End of #if (MCU_NO_OF_STDBY_RAM_BLK != 0U) */
...
LOCAL_INLINE Std_ReturnType Mcu_lGtmGlobalInitCheck(void)
{
  ...
  if (CmpVal == 0xFFFFFFFFU)
  {
    ...
    ConfigVal = Mcu_ConfigPtr->McuGtmConfigPtr->GtmTbuCfg & MCU_TBU_ENDIS_CH_MASK;
    CmpVal   &= ~(SfrVal ^ ConfigVal);
...
#endif
/* End of #if ((MCU_GTM_USED == STD_ON) && (MCU_INITCHECK_API == STD_ON)) */
