...
void EB_TpRxMsgManageFullDuplex (void)
{
    ...
    if ( EB_TRUE == m_uEbTpMsgFlagIdx[!"name(.)"!])
    {
        ...
        if((EB_STATUS_OK == eEbQueuedStatus) || (EB_STATUS_QUEUED == eEbQueuedStatus))
        {
            ...
            eStatusRxReq = UDS_RxRequestWithAddrMode (&m_uwLen, m_paubUdsData, [!IF "Adressing_Mode = 'Physical'"!]UDS_ADDR_PHYSICAL[!ELSE!]UDS_ADDR_FUNCTIONAL[!ENDIF!], [!IF "Type = 'Diag'"!]UDS_TYPE_DIAG[!ELSEIF "Type = 'Obd'"!]UDS_TYPE_OBD[!ELSEIF "Type = 'Diag_Obd'"!]UDS_TYPE_DIAG_OBD[!ENDIF!]);
