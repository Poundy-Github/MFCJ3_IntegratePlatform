...
NCS_PRI_VAR const u8 * m_pubDataBuffer;
NCS_PRI_VAR u32 m_uwDataLength;
#if (BM_CSM_HASH_USED == STD_ON)
NCS_PRI_VAR u8 * m_pubChecksum;
#elif (BM_CSM_MAC_USED == STD_ON)
NCS_PRI_VAR const u8 * m_pubChecksum;
#endif
#endif
#endif
#endif
...
NCS_PRI_VAR Csm_VerifyResultType m_eCsmResult = CSM_E_VER_NOT_OK;
...
#if (BM_CSM_ASR43_USED == STD_ON)
#if (BM_BLOC_SLICING_ENABLED == STD_ON)
            if (uwDataLength > BM_BLOC_SLICING_SIZE)
            {
                uwSlicedDataLength = BM_BLOC_SLICING_SIZE;
                m_uwDataLength -= BM_BLOC_SLICING_SIZE;
            }
            else
            {
...
#endif
{
    ...
    if ((CSM_E_OK == m_ubCsmResult) && (m_uwDataLength > 0U))
    {
        ...
        if (m_uwDataLength > BM_BLOC_SLICING_SIZE)
        {
