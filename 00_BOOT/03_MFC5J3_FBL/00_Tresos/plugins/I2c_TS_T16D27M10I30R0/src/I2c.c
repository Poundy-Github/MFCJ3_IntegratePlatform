...
static I2c_ErrorType I2c_ReadWriteDetCheck(const I2c_ChannelType ChannelId,
                ...
                const I2c_SlaveAddrType SlaveAddress, uint8 ServiceId);
#endif
...
)
{
  ...
  {
    for(Channel = I2C_ZERO; Channel < ConfigPtr->I2c_MaxChannels; Channel++)
    {
...
)
{
  ...
  #endif
  {
    ...
    for(Channel = I2C_ZERO; Channel < I2C_MAX_CHANNELS; Channel++)
    {
      ...
      I2C_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG(&HwModulePtr->CLC.U,I2C_ONE);
    }
...
)
{
  ...
  #endif
  {
    ...
    if(ChannelConfigPtr->FracDividerHighCfgValue > I2C_ZERO)
    {
    ...
    {
      HwModulePtr->TPSCTRL.U = ((uint32)(ChannelInfo->DataLeft) + (uint32)I2C_ONE);
    }
    else
    {
      HwModulePtr->TPSCTRL.U = ((uint32)(ChannelInfo->DataLeft) + (uint32)I2C_TWO);
    }
...
)
{
  ...
  #endif
  {
    ...
    if(ChannelConfigPtr->AddressingMode == I2C_7_BIT_ADDRESSING)
    {
      HwModulePtr->TPSCTRL.U = I2C_ONE;
    }
    else
    {
      HwModulePtr->TPSCTRL.U = I2C_TWO;
    }
...
)
{
  ...
  I2C_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG(&HwModulePtr->CLC.U, (uint32)I2C_ZERO);
  ClkDisableChk = HwModulePtr->CLC.B.DISS;
  if(ClkDisableChk == I2C_ZERO)
  {
...
)
{
  ...
  HwModulePtr->GPCTL.U = ChannelConfigPtr->PortPinCfgvalue;
}
...
)
{
  ...
  KRSTValue = (((HwModulePtr->KRST0.U) & I2C_KRST0_SFR_MSK) | \
               ((uint32)I2C_ONE << IFX_I2C_KRST0_RST_OFF));
  I2C_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG(&HwModulePtr->KRST0.U, KRSTValue);
  I2C_INIT_DEINIT_WRITE_PERIP_ENDINIT_PROTREG\
...
)
{
  ...
  if (I2c_InitStatus == I2C_UNINITIALIZED)
    ...
    else if((Size == I2C_ZERO) || (Size > I2C_MAX_SUPPORTED_SIZE))
    {
      /* Report to DET */
      Det_ReportError(I2C_MODULE_ID, I2C_INSTANCE_ID,
...
)
{
  ...
  if(TxdCnt == I2C_ZERO)
  {
    ...
    else
    {
      ...
      if((ChannelInfo->DataLeft & 3u) > I2C_ZERO)
      {
         FilledFIFOStage++;
      }
    }
  ...
  {
    if((HwModulePtr->RIS.U & I2C_BURST_REQ_CHK) != I2C_ZERO)
    {
  ...
  while(FIFOCounter < FilledFIFOStage)
  {
    ...
    while((Counter < I2C_FIFO_MAX_DATA_IN_EACH_STAGE) && (DataLeft > I2C_ZERO))
    {
...
)
{
  ...
  while((TransmitPending == I2C_TX_IN_PROGRESS) && (TimeOutValue > I2C_ZERO))
  {
    ...
    if((HwModulePtr->RIS.U & I2C_RAW_INTERRUPT_MASK) != I2C_ZERO)
    {
      I2c_lWrite(HwModulePtr,ChannelInfo);
      HwModulePtr->ICR.U = I2C_CLEAR_INTERRUPT;
    }
    /* Get the error id if any error has occured */
    else if((ERRIRQSS_Val != I2C_ZERO) || (PIRQSS_Val != I2C_ZERO))
    {
        ChannelInfo->HwState = I2C_IDLE;

        if(HwModulePtr->ERRIRQSS.B.TXF_UFL == I2C_ONE)
        {
          ErrorId = I2C_TX_UNDERFLOW;
        }
        else if(HwModulePtr->ERRIRQSS.B.TXF_OFL == I2C_ONE)
        {
          ErrorId = I2C_TX_OVERFLOW;
        }
        else if(HwModulePtr->PIRQSS.B.NACK == I2C_ONE)
        {
          ErrorId = I2C_NO_ACK;
        }
        else if(HwModulePtr->PIRQSS.B.AL == I2C_ONE)
        {
          ErrorId = I2C_ARBITRATION_LOST;
        }
        else if(HwModulePtr->PIRQSS.B.TX_END == I2C_ONE)
        {
          ErrorId = I2C_NO_ERR;
        }
        else
...
)
{
  ...
  if(ChannelInfo->RxTransferState == I2C_RX_INITIATED)
  {
    ...
    if(I2c_ConfigPtr->I2c_ChannelConfigPtr->AddressingMode ==
                                                           I2C_7_BIT_ADDRESSING)
    {
      TxData = (((uint32)SlaveAddr << (uint32)I2C_ONE) | (uint32)I2C_READ_OPERATION);
      HwModulePtr->TXD.U = TxData;
    }
    else
    {
      /* Calculate the upper slave byte */
        ...
        HwModulePtr->ENDDCTRL.B.SETRSC = I2C_ONE;
  ...
  else
  {
    ...
    for (LoopCount = I2C_ZERO; LoopCount < FilledFIFOStage; LoopCount++)
    {
...
)
{
  ...
  while((ReceivePending == I2C_RX_IN_PROGRESS) && (TimeOutValue > I2C_ZERO))
  {
    /* Get the error values if any */
    ERRIRQSS_Val = HwModulePtr->ERRIRQSS.U & I2C_ERROR_INTERRUPT_MASK;
    PIRQSS_Val = HwModulePtr->PIRQSS.U & I2C_PROTOCOL_INTERRUPT_MASK;
    if((ERRIRQSS_Val != I2C_ZERO) || (PIRQSS_Val != I2C_ZERO))
    {

      if(HwModulePtr->ERRIRQSS.B.RXF_UFL == I2C_ONE)
      {
         ErrorId = I2C_RX_UNDERFLOW;
      }
      else if(HwModulePtr->ERRIRQSS.B.RXF_OFL == I2C_ONE)
      {
    ...
    else if((HwModulePtr->RIS.U & I2C_RAW_INTERRUPT_MASK) != I2C_ZERO)
    {
...
)
{
  ...
  {
    if(ChannelInfo->DataLeft > I2C_ZERO)
    {
       I2c_lWrite(HwModulePtr, ChannelInfo);
    }
    HwModulePtr->ICR.U = I2C_CLEAR_INTERRUPT;
...
)
{
  ...
  if((HwModulePtr->PIRQSS.U & I2C_PROTOCOL_INTERRUPT_MASK) != I2C_ZERO)
  {
    if(ChannelInfo->RxTransferState != I2C_RX_COMMAND_SENT)
    {
      if(HwModulePtr->PIRQSS.B.NACK == I2C_ONE)
      {
        ErrorId = I2C_NO_ACK;
      }
      else if(HwModulePtr->PIRQSS.B.AL == I2C_ONE)
...
)
{
  ...
  if((HwModulePtr->ERRIRQSS.U & I2C_ERROR_INTERRUPT_MASK) != I2C_ZERO)
  {
    ChannelInfo->HwState = I2C_IDLE;
    ...
    else if(HwModulePtr->ERRIRQSS.B.RXF_UFL == I2C_ONE)
    {
       ErrorId = I2C_RX_UNDERFLOW;
    }
    else if(HwModulePtr->ERRIRQSS.B.RXF_OFL == I2C_ONE)
