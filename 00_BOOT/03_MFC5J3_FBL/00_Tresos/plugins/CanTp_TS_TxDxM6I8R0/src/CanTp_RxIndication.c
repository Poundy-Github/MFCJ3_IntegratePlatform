    ...
    if(CanTp_FindNSduId(CanTpRxPduId, CanTpRxPduPtr->SduDataPtr, &NSduId, &FrameType))
    {
...
#endif /* CANTP_PARALLEL_CHANNELS_ENABLED == STD_ON */
      {
        ...
        if(FrameType == CANTP_N_PCI_TYPE_FC)
        {
          ...
          if(CANTP_CFG_GET_TXNSDU_CONFIG(NSduId).PaddingActivation == CANTP_ON_CAN_CAN_FD)
          {
        ...
        if(CanTpRxPduPtr->SduLength <= NPduMaxLength)
        {
          P2CONST(uint8, AUTOMATIC, CANTP_APPL_CONST) SduDataPtr = CanTpRxPduPtr->SduDataPtr;
          CONSTP2CONST(CanTp_RxNSduConfigType, AUTOMATIC, CANTP_APPL_CONST)
          RxNSduCfgPtr = &CANTP_CFG_GET_RXNSDU_CONFIG(NSduId);
          ...
          if((FrameType == CANTP_N_PCI_TYPE_FF) || (FrameType == CANTP_N_PCI_TYPE_SF))
          {
            ...
            if(CANTP_GET_ADDR_FORMAT(RxNSduCfgPtr) == CANTP_STANDARD)
            {
              N_AI = CanTpRxPduId;
            }
            else
            {
              N_AI = (PduIdType)SduDataPtr[0U];
            }

            if(ChannelPtr->RxChannelLockFlag == (boolean)FALSE)
            {
          ...
          if(IgnoreMsgFlag == (boolean)FALSE)
          {
            ...
            if(
                ...
                ((FrameType == CANTP_N_PCI_TYPE_SF) && (ChannelPtr->TxLastFrameFlag == TRUE)))
            )
            {
#if(CANTP_DEV_ERROR_DETECT == STD_ON)
              if((FrameType == CANTP_N_PCI_TYPE_CF) && (ChannelPtr->PostRxIndFlag == (boolean)TRUE))
              {
                OverwrittenCF = TRUE;
              }
#endif /* (CANTP_DEV_ERROR_DETECT == STD_ON) */
...
#if(CANTP_DEV_ERROR_DETECT == STD_ON)
  if((AddressingFormat == CANTP_STANDARD) && (FoundFlag == (boolean)FALSE))
  {
    CANTP_DET_REPORT_ERROR(
                            CANTP_MODULE_ID,
                            CANTP_INSTANCE_ID,
...
)
{
  ...
  if(CanTpRxPduPtr->SduLength <= CANTP_NPDU_LENGTH_8)
  {
    ...
    {
      /* for STANDARD addressing, SF_DL shall be smaller or equal than CAN_DL-1 (see Table 12 from ISO 15765-2:2016) */
  ...
  else if((CanTpRxPduPtr->SduDataPtr[StartPosition_SF_N_PCI] & CANTP_N_PCI_SF_DL_MASK) == 0U) /* low nibble SF_DL is 0 and CAN_DL > 8 => CAN FD single frame */
  {
    ...
    else /* SF_DL greater or equal than 8 => CAN FD single frame format */
    {
      ...
      /* for STANDARD addressing, SF_DL shall be smaller or equal than CAN_DL-2 (see Table 13 from ISO 15765-2:2016) */
      if(StartPosition_SF_N_PCI == 0U)
      {
     ...
     if(Ret == E_OK)
     {
       ...
       switch(Ret_ReqRxBuffer)
       {
         ...
         case BUFREQ_E_BUSY:
         {
           ...
           ChannelPtr->RxChannelLockFlag = TRUE;
...
)
{
  ...
  if((ChannelState == CANTP_CH_RX_RECEIVE_CF) || (ChannelState == CANTP_CH_RX_CONFIRM_FC_CTS))
  {
    ...
    else
    {
      ...
      {
        ChannelPtr->TxLastFrameFlag = TRUE;
    ...
    if(BUFREQ_OK == Ret_CopyRxData)
    {
      ...
      if(ChannelPtr->ProcessedBlockFrames == ChannelPtr->BlockSize)
      {
        ...
        else
        {
           ...
           */
          ChannelPtr->Ticks =
            CANTP_GET_RX_PDU_N_BR(&CANTP_CFG_GET_RXNSDU_CONFIG(ChannelPtr->NSduId))
...
)
{
  ...
  if(ValidFrameLength)
  {
    ...
    ChannelPtr->TxLastFrameFlag = FALSE;
  }
#if(CANTP_DEV_ERROR_DETECT == STD_ON)
  else
  {
  ...
  switch(ChannelState)
  {
    ...
    case CANTP_CH_TX_CONFIRM_CF:
    {
      ...
      /* If padding is enabled the NPdu length shall be filled with padding bytes to
          the maximum of 8.
       */
      if(CANTP_CFG_GET_TXNSDU_CONFIG(ChannelPtr->NSduId).PaddingActivation == CANTP_ON)
      {
        NPduLength = CANTP_NPDU_LENGTH_8;
      }
      else if(CANTP_CFG_GET_TXNSDU_CONFIG(ChannelPtr->NSduId).PaddingActivation == CANTP_ON_CAN_CAN_FD)
      {
        /* CAN FD NPdu => NPdu length shall be filled with padding bytes
         * to the maximum of 64 bytes */
        NPduLength = CANTP_CFG_GET_TXNSDU_CONFIG(ChannelPtr->NSduId).LSduMaxLength;
      }
      else
      {
        if(CanTpRxPduPtr->SduLength == CANTP_NPDU_LENGTH_8)
        {
          /* Also accept padded frames even if padding is disabled (see enhancement). */
          NPduLength = CANTP_NPDU_LENGTH_8;
        }
        else
        {
          /* N-PDU length must be the address byte (if exist) + FC N_PCI header */
          NPduLength = StartPosition_FC_N_PCI + CANTP_FC_NPCI_SIZE;
        }
      }

      if(CanTpRxPduPtr->SduLength == NPduLength)
      {
        ValidFrameLength = TRUE;
        ...
        switch(CanDataPtr[StartPosition_FC_N_PCI])
        {
          case CANTP_N_PCI_FC_CTS:
          {
            ...
            if (ChannelPtr->StMin != 0u)
            {
              ...
              ChannelPtr->STminRoundUpFlag = TRUE;
            }
...
)
{
  ...
  CanTp_ChType idx;
  CanTp_ChStateType CurrentState;
  CanTp_TxChannelType NoOfChannels = CANTP_MAX_TX_CHANNELS;
