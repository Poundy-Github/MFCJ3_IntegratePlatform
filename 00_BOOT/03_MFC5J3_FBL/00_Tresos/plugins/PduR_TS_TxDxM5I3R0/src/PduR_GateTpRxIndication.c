...
)
{
      ...
      else
      {
         ...
         if(PduR_GateTpNoSessionQueued(GMPduId))
         {
            ...
            if(ResultGetBitTxComplete == 0U)
            {
               ...
               /* release queue element and update head idx will be done after CallLoTpTransmit */
...
#else
            /* if no TP GW queuing PduR_ReleaseGateTpQueueElement() should not be called at all */
            ReleaseQueueElement = FALSE;
#endif /* (PDUR_TPGWQUEUE_ENABLE == STD_ON) */
         }
   ...
   if(CallLoTpTransmit)
   {
     /* no correct Transmit call initially */
      ...
      {
         /* TODO: verify if this really needs to be within a critical section for multicore */
