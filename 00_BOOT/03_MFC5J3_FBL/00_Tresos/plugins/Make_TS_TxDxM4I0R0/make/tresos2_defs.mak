...
override TRESOS2_OUTPUT_PATH   := $(subst \,/,$(GEN_OUTPUT_PATH))
# set tresos batch file
TRESOS2_GENERATOR     := $(TRESOS_BIN)\tresos_cmd.bat
...
$(foreach plugin,$(TRESOS2_ALL_PLUGINS),$(eval TRESOS2_$(plugin)_CONF_FILES = $(filter $(addprefix %/,$(filter $(plugin).xdm $(plugin)_%.xdm,$(notdir $(TRESOS2_CONF_FILES)))),$(TRESOS2_CONF_FILES))))
...
#
TRESOS2_CONF_MODULE_IDS   = $(sort $(PLUGINS_TO_GENERATE) $(if $(Resource_VARIANT),Resource_$(Resource_VARIANT)) $(foreach MOD,$(TRESOS2_ADDITIONAL_RSC_MODULES),$(if $($(MOD)_VARIANT),$(MOD)_$($(MOD)_VARIANT))))
...
ifeq (,$(TRESOS2_PROJECT_NAME))
  ...
  TRESOS2_SYSD_GENSWCD_FILES ?= $(addsuffix @sysd,$(wildcard $(PROJECT_ROOT)/config/generate_swcd/*.arxml))

  # generated sysd files
  TRESOS2_SYSD_GENERATED_FILES := $(PROJECT_ROOT)/output/generated/swcd/*.arxml

  # static sysd files
  TRESOS2_SYSD_ALL_FILES ?= $(addsuffix @sysd,$(wildcard $(PROJECT_ROOT)/config/*.arxml))
...
override TRESOS2_BATCH_FILE  := $(subst \,/,$(TRESOS2_OUTPUT_PATH)/tresos_cmd.txt)
