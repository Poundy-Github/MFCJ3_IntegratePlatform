...
#define GPT_GTM_TOM_PREDEFTIMER_MAX     ((uint32)0xFFFFU)
#endif
...
/*[cover parentID={426DF98B-92C4-4c34-8784-BB277D66F468}][/cover]*/
...
#if ((GPT_SAFETY_ENABLE == STD_ON) || (GPT_DEV_ERROR_DETECT == STD_ON))
/* API to check for the DET’s for the API’s Gpt_EnableNotification and
...
void Gpt_StartTimer(const Gpt_ChannelType Channel, const Gpt_ValueType Value )
{
                                            ...
                                            (GPT_DEV_ERROR_DETECT == STD_ON)) */
  {
        ...
        Gpt_ChannelStatusPtr[LocalChannel] == GPT_CHANNEL_RUNNING)
    {
      ...
      Report DET
      [/cover] */
      Gpt_lReportError(GPT_SID_STARTTIMER, GPT_E_BUSY);
    }
    else
    #endif
    {
...
Gpt_ValueType Gpt_GetTimeElapsed(const Gpt_ChannelType Channel )
{
  ...
  IDetVal = Gpt_lGetTimeDetCheck(Channel, GPT_SID_GET_TIMEELAPSED,
                                 CurrentCoreId);
                                              ...
                                              (GPT_DEV_ERROR_DETECT == STD_ON)) */
  {
    ...
    [/cover] */
    if (Gpt_lCheckChannelStatus(LocalChannel, CurrentCoreId) == (boolean)TRUE)
    {
...
Gpt_ValueType Gpt_GetTimeRemaining(const Gpt_ChannelType Channel)
{
  ...
  Check for DET errors
  [/cover] */
  IDetVal = Gpt_lGetTimeDetCheck(Channel, GPT_SID_GET_TIMEREMAINING, \
                                 CurrentCoreId);
                              ...
                              (const Gpt_ChannelConfigType *const GptChannelPtr)
{
  ...
  uint8 LocalGpt12TimerId0 = GptChannelPtr->GptGpt12TimerInfo[0].TimerId;
...
static uint32 Gpt_lDeInitDetCheck(const uint32 CurrentCoreId)
{
  ...
  CurrentCoreDriverState = \
                           Mcal_GetBitAtomic(Gpt_DriverStatus, GPT_CORE_BITPOS(CurrentCoreId), \
                               GPT_CORE_STATUS_LEN);
  /* [cover parentID={A8C6911D-E812-4f3a-A788-3CD693847C01}]
  Is safety or Multicore Error reporting is enabled?
  [/cover] */
  ...
  #endif
  {
    ...
    else
    {
      ...
      if (IErrVal == GPT_E_DET_ERR)
      {
        ...
        Report DET
        [/cover] */        /* Report to  DET */
        Gpt_lReportError(GPT_SID_DEINIT, GPT_E_BUSY);
      }
    }
  }
                                      ...
                                      )
{
  ...
  {
    #if ((GPT_SAFETY_ENABLE == STD_ON) || (GPT_DEV_ERROR_DETECT == STD_ON))
    /* [cover parentID={99E19940-C65D-41ac-82FB-2EF44F84D7DB}]
    Report to DET
    [/cover] */
    Gpt_lReportError(GPT_SID_GET_PREDEFTIMERVALUE, GPT_E_MODE);
    #endif
                                ...
                                )
{
  ...
  if (LocalGptGtmTimerInfo->TimerType == MCU_GTM_TIMER_TOM)
  {
    ...
    if (Gpt_lCheckTimeOutVaue(TimeOutVal, GPT_GTM_TOM_TIMER_MAX) == TRUE)
    {
      ...
      /* [cover parentID={67A97636-7CBD-4d55-936D-4B0B3CDE643C}]
      Start of critical section
      [/cover] */
      SchM_Enter_Gpt_GtmStartTimer();
      
      /* [cover parentID={38A1962E-B2C1-4c69-960D-3408B12D2FEA}]
      ...
      /* [cover parentID={7A7EF744-57BF-4479-9D62-249499E15AF5}]
      ...
      /* [cover parentID={B551E4E6-BBC5-4735-AC9C-E33AE14E7311}]
  ...
  if (LocalGptGtmTimerInfo->TimerType == MCU_GTM_TIMER_ATOM)
  {
    ...
    if (Gpt_lCheckTimeOutVaue(TimeOutVal, GPT_GTM_ATOM_TIMER_MAX) == TRUE)
    {
      ...
      /* [cover parentID={A74D7EDC-27CE-44b7-96F9-069E75CEB1DE}]
      Start of critical section
      [/cover] */
      SchM_Enter_Gpt_GtmStartTimer();
                                ...
                                )
{
   ...
   * this MISRA rule, Gpt12TxRegPtr1 is used to access the SFR. 
   * Union is required */
  ...
  uint8 Timernumber;                     /* Holds the channel number     */
  uint8 Timernumber2;                    /* Holds the channel number     */
  uint32 Ctrlregval;
                            ...
                            (const Gpt_ChannelConfigType *const GptChannelPtr)
{
  ...
  if (LocalGptGtmTimerInfo->TimerType == MCU_GTM_TIMER_TOM)
  {
    ...
    /* [cover parentID={2C8E0030-491D-4d2a-BD12-3B7D78DA88CB}]
  ...
  if (LocalGptGtmTimerInfo->TimerType == MCU_GTM_TIMER_ATOM)
  {
    ...
    /* [cover parentID={3A050702-6B28-4536-B2C5-6CECA00AC5A4}]
      Set CN0 equal to CM0
      [/cover] */
...
static void Gpt_lReportError(const uint8 Gpt_Sid, const uint8 ErrorId)
{
  ...
  Report DET
  [/cover] */
  Det_ReportError(GPT_MODULE_ID, GPT_INSTANCE_ID, Gpt_Sid, ErrorId);
  #endif
