...
define build-exe-file
	$(info $(addprefix -l, $(notdir $(basename $(LIBRARIES_TO_BUILD) $(LIBRARIES_LINK_ONLY)))))
	$(EB_VERBOSE)$(GPP) -o $@ @$(LNK_INC) -Wl$(COMMA)--start-group $(addprefix -l, $(notdir $(basename $(LIBRARIES_TO_BUILD) $(LIBRARIES_LINK_ONLY)))) -Wl$(COMMA)--end-group $(LINK_OPT_POST)
	@echo *** BUILD DONE ***
endef
...
define	build-cpp-file
	...
	$(file > $(DEPEND_DIR)/$(notdir $(basename $@)).d, DUMMY_VAR=compile error in c file $(notdir $<))
	$(EB_VERBOSE)$(GPP) @$(DEPEND_INC) -MM -MF $(subst \,/,$(DEPEND_DIR)\$(notdir $(basename $@)).d) -MT $(subst \,/,$(OBJ_DIR)/$(notdir $(basename $@)).o)  $(realpath $<) 
	$(EB_VERBOSE)$(GPP) @$(subst \,/,$(MAKE_INCPP)) -c -o $(OBJ_DIR)/$(notdir $(basename $@)).o $(subst /,\,$<)
endef
