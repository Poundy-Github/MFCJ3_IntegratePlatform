 ...
 *
 * Reason:
 * according to MISRA-C a switch statement has to have a default branch, even
 * it can never be reached. Therefore it cannot be covered by a test case.
 *
 *
 * NOCOV-DEFENSIVE
 *
 * Reason:
 * Defensive programming. This case can never be reached. Therefore it cannot
 * be covered by a test case.
 *
 *
 */
...
NCS_PRI_VAR tUdsSessionIndex m_eSessionIdx;
#else
/* Current session code (DSC service parameter) */
...
NCS_PRI_VAR tUdsBoolean m_eUdsObdService;
#endif
...
NCS_PRI_VAR tUdsLongRequestStr m_stLongRequest;
#endif
 ...
 ** m_astDiagSrvCfg1.
 **
 ** \param[in] puwLen pointer on data length (request)
 ** \param[in] aubUdsData pointer on data (request)
 **
 ** \return Diagnostic response status
 ** \retval UDS_ACK     Positive response
 ...
 **
 **/
NCS_PRI_FCT tUdsStatus UDS_SubServiceSessionFiltering1 (const u16 *puwLen, const u8 *aubUdsData);
#endif
#if (UDS_SRV_CFG2_MAX !=0)
 ...
 **
 **/
NCS_PRI_FCT tUdsStatus UDS_DidSessionFiltering2(u16 *puwLen, const u8 *aubUdsData);
 ...
 **
 **/
NCS_PRI_FCT tUdsStatus UDS_RiSessionFiltering3 (const u16 *puwLen, const u8 *aubUdsData);
 ...
 ** \retval UDS_FALSE  puwLen and/or aubUdsData are NULL pointers
 ** \retval UDS_TRUE  when puwLen is different that zero
 **
 **/
 ...
 **
 ** \param[in] puwLen const pointer on data length (request)
 ...
 **
 ** \param[in] uwLen Length of the data to send
 ...
 **
 **/
NCS_PRI_FCT tUdsStatus UDS_LongRequestResponseInd (u16 uwLen, u8 *aubUdsData);
#endif
 ...
 **
 **/
NCS_PRI_FCT tUdsStatus UDS_RxSupplierRequest (u16 *puwLen, u8 *aubUdsData, tUdsAddrMode eUdsAddrMode, tUdsAddrType eUdsAddrType);
#endif
...
tUdsBoolean UDS_RxRequest (u16 *puwLen, u8 *aubUdsData)
{
    ...
    DBG_UDS_RXREQUEST_ENTRY(puwLen,aubUdsData);
...
tUdsStatus UDS_RxSupplierRequest (u16 *puwLen, u8 *aubUdsData, tUdsAddrMode eUdsAddrMode, tUdsAddrType eUdsAddrType)
{
    ...
    switch (aubUdsData[0])
    {
        ...
        case UDS_SRV_BA:
            eUdsStatus = UDS_CustomSupplier_BA (puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
            break;
#endif

#if (UDS_SUPPLIERSERVICE_BB == STD_ON)
        case UDS_SRV_BB:
            eUdsStatus = UDS_CustomSupplier_BB (puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
            break;
#endif

#if (UDS_SUPPLIERSERVICE_BC == STD_ON)
        case UDS_SRV_BC:
            eUdsStatus = UDS_CustomSupplier_BC (puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
            break;
#endif

#if (UDS_SUPPLIERSERVICE_BD == STD_ON)
        case UDS_SRV_BD:
            eUdsStatus = UDS_CustomSupplier_BD (puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
            break;
#endif

#if (UDS_SUPPLIERSERVICE_BE == STD_ON)
        case UDS_SRV_BE:
            eUdsStatus = UDS_CustomSupplier_BE (puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
            break;
#endif
...
tUdsBoolean UDS_RxRequestWithAddrMode (u16 *puwLen, u8 *aubUdsData, tUdsAddrMode eUdsAddrMode, tUdsAddrType eUdsAddrType)
{
    ...
    DBG_UDS_RXREQUESTWITHADDRMODE_ENTRY(puwLen,aubUdsData,eUdsAddrMode,eUdsAddrType);
    ...
    (UDS_SUPPLIERSERVICE_BE == STD_ON) )
                ...
                case UDS_SRV_BE:
                    ...
                    eUdsStatus = UDS_RxSupplierRequest(puwLen, aubUdsData, eUdsAddrMode, eUdsAddrType);
                    break;
#endif
        ...
        if(eUdsStatus == UDS_ACK)
        {
            ...
            if(eUdsStatus == UDS_ACK)
            {
                switch (aubUdsData[0])
                {
                    ...
                    case UDS_SRV_LC:
                        ...
                        if (*puwLen >= 2U)
                        {
                            ...
                            if(eUdsStatus == UDS_ACK)
                            {
                                ...
                                eUdsStatus = UDS_SubServiceSessionFiltering1(puwLen, aubUdsData);
                            }
                            else
                            {
                                /* Nothing to do */
                    ...
                    case UDS_SRV_TD:
                        /* NRC13 (Minimal =  3) / incorrectMessageLengthOrInvalidFormat / IMLOIF */
                        if ((*puwLen < 3U) || (*puwLen > PROG_MAX_BYTE_IN_TD))
                        {
                            eUdsStatus = UDS_NRC_13;
                        }
                        break;
                ...
                if(eUdsStatus == UDS_ACK)
                {
...

                    {
                        ...
                        if ((*puwLen == m_astDiagSrvCfg1[m_uSrvIdx].uwReqLen) || (m_astDiagSrvCfg1[m_uSrvIdx].uwReqLen == 0U))
                        {
                            ...
                            if (m_astDiagSrvCfg1[m_uSrvIdx].pfuRxMsgInd != NULL_PTR)
                            {
                                ...
                                /* Individual callback for each service */
                                eUdsStatus = m_astDiagSrvCfg1[m_uSrvIdx].pfuRxMsgInd(puwLen, aubUdsData);
    ...
    else
    {
        ...
        if(eUdsStatus == UDS_ACK)
        {
            ...
            if(eUdsStatus == UDS_ACK)
            {
                ...
                {
                    case UDS_SRV_RDBI:
                        eUdsStatus = UDS_RdbiManagement(puwLen,aubUdsData);
                        break;
                    ...
                    case UDS_SRV_RSDBI:
                        ...
                        if (*puwLen >= uwMinLength)
                        {
                            ...
                            {
                                /* Session Filtering */
                                eUdsStatus = UDS_DidSessionFiltering2(puwLen, aubUdsData);

                                if(eUdsStatus == UDS_ACK)
                                {
                                    if ((*puwLen == m_astDiagSrvCfg2[m_uSrvIdx].uwReqLen) || (m_astDiagSrvCfg2[m_uSrvIdx].uwReqLen == 0U))
                                    {
#if (UDS_SECURITY_CHECK == UDS_SECURITY_CHECK_ON)
                                        /* NRC33 / securityAccessDenied / SAD */
...
{
    tUdsStatus eUdsStatus;
    u8 ubTableCfgIdx[UDS_SRV_CFG2_MAX] = {0};
    u8 ubTableCfgLength;
    u16 uwIdx;
    u8 ubLocalDidIdx;
    u16 ubLocalLength;
    u8 ubLastDataInPreviousDID;

    eUdsStatus = UDS_ERR_COHE;
    ubLastDataInPreviousDID = UDS_SRV_RDBI;
    ubLocalLength = 0U;
    ubTableCfgLength = 0U;
    uwIdx = 1U;
    ...
    if (((*puwLen >= 3U) &&
        (((*puwLen - 1U) % 2U) == 0U)) &&
        (*puwLen <= (1U + (2U*UDS_SRV_CFG_RDBI_MAX))))
    {
        /* Loop on all the DIDs into the request */
        while ((uwIdx < (*puwLen - 1U)) && (eUdsStatus != UDS_NRC_33))
        {
            /* Retrieve the next DID */
            m_uwDid = (u16)((u16)((u16)aubUdsData[uwIdx] << 8U) + ((u16)aubUdsData[uwIdx+1U]));
            uwIdx += 2U;
...
#endif
                    {
                        ubTableCfgIdx[ubTableCfgLength] = m_uSrvIdx;
                        ubTableCfgLength++;
                    }
                }
            }
        ...
        if(eUdsStatus != UDS_NRC_33)
        {
            ...
            if (0U != ubTableCfgLength)
            {
                ...
                while (((uwIdx < ubTableCfgLength) && (UDS_ACK == eUdsStatus)) && (*puwLen > 0U))
                {
                    ubLocalDidIdx = ubTableCfgIdx[uwIdx];
                    ...
                    aubUdsData[ubLocalLength] = UDS_SRV_RDBI;
                    aubUdsData[ubLocalLength+1U] = (u8)((u8)(m_astDiagSrvCfg2[ubLocalDidIdx].uwDid >> 8U) & 0xFFU);
                    aubUdsData[ubLocalLength+2U] = (u8)(m_astDiagSrvCfg2[ubLocalDidIdx].uwDid & 0xFFU);
                    if (m_astDiagSrvCfg2[ubLocalDidIdx].pfuRxMsgInd != NULL_PTR)
                    {
                        /* Save LocalLength (total data in buffer) in puwLen */
                        *puwLen = ubLocalLength;

                        /* Individual callback for each service */
                        eUdsStatus =
                            m_astDiagSrvCfg2[ubLocalDidIdx].pfuRxMsgInd(puwLen, &aubUdsData[ubLocalLength]);

                        /* Length = 1 or 2 so SRV (1)+ DID (2) are taken into account -> 3 */
                        if ((*puwLen == 1U) || (*puwLen == 2U))
                        {
                            *puwLen= 3U;
                        }
                    }
                    ...
                    if (*puwLen > 0U)
                    {
                        /* Restore the last byte into the response for the previous DID */
                        aubUdsData[ubLocalLength] = ubLastDataInPreviousDID;
                        /* Update the global response length */
                        ubLocalLength += (*puwLen - 1U);
                        /* Memorize the last byte of the current response. It will be overwritten by SRV on next loop. */
                        ubLastDataInPreviousDID = aubUdsData[ubLocalLength];
                        uwIdx++;
                    }
                }
                /* Length = 0 no UDS response expected */
        ...
        if(eUdsStatus == UDS_ACK)
        {
            ...
            if(eUdsStatus == UDS_ACK)
            {
                ...
                if (*puwLen >= 4U)
                {
                    ...
                    if(eUdsStatus == UDS_ACK)
                    {
                        eUdsStatus = UDS_RiSessionFiltering3(puwLen, aubUdsData);
...
#endif
{
    ...
    else
    {
        ...
        if(*puwLen != 0U)
        {
            ...
            {
                UDS_CustomPositiveAnswerInd(puwLen,aubUdsData);
            }
        }
        else
        {
...
{
    DBG_UDS_LONGREQUESTEND_ENTRY(uwLen,aubUdsData,eStatus);
    ...
    if ((m_stLongRequest.m_eState == UDS_LONG_REQ_ON) && (aubUdsData != NULL_PTR))
    {
        ...
        if(eStatus==UDS_ACK)
        {
            ...
            {
                /* This is a positive answer, transform the service identifier */
                (void)UDS_PositiveAnswer(&uwLen, aubUdsData);
            }
        }
        else
        {
    ...
    switch (m_stLongRequest.m_eState)
    {
        case UDS_LONG_REQ_END:
            if (UDS_FALSE == m_stLongRequest.m_ubtRespPending)
            {
                ...
                (void)UDS_LongRequestResponseInd (m_stLongRequest.m_uwLen, m_stLongRequest.m_paubData);
            }
            break;
...
NCS_PRI_FCT tUdsStatus UDS_LongRequestResponseInd (u16 uwLen, u8 *aubUdsData)
{
    BlPduR_Send_TPFrame(uwLen, aubUdsData);
